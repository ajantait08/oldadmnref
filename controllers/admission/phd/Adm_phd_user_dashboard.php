...
class Adm_phd_user_dashboard extends MY_Controller {
  ...
  public function education_apply_post()//for deleting temp file
  {
    ...
    if($p_row_id==$row_id)
    {
      ...
      $this->Add_phd_registration_model->delete_specialization($specialization);
  ...
  public function submit()
  {
    ...
    $datac['cfti_yes']=$this->Add_phd_registration_model->get_cfti_by_reg($get_reg_no);
  ...
  public function apply_post()
  {
    ...
    if(!empty($get_reg_details[0]->color_blind))
    {
      ...
      {
        $data['btech_paper']=$this->Add_phd_registration_model->get_programme_list_of_btech_without_pwd();
      ...
      {
        $data['btech_paper']=$this->Add_phd_registration_model->get_programme_list_of_btech_without_pwds();
  ...
  public function get_apply_post()
  {
    ...
    if ($this->form_validation->run() == true)
    {
      ...
      $this->Adm_phd_application_model->insert_specailization($prio1);

      if(!empty($Priority2))
      {
        $this->Adm_phd_application_model->insert_specailization($prio2);
      }

      if(!empty($Priority3))
      {
        $this->Adm_phd_application_model->insert_specailization($prio3);
      }
  ...
  public function get_program_elligibilty()
  {
    ...
    $this->load->model('admission/phd/Add_phd_registration_model');
    $programme_apply_for = $this->input->post('programme_apply_for');
    $dept_ids=$this->Add_phd_registration_model->get_department_id_by_program_id($programme_apply_for);
    $data['prog'] = $this->Add_phd_registration_model->program_phd($dept_ids);
  ...
  public function get_specialization_by_program_id()
  {
    $this->load->model('admission/phd/Adm_phd_personal_details_model');
    $programme_apply_for = $this->input->post('programme_apply_for');
   $data['specail'] = $this->Adm_phd_personal_details_model->get_specialization($programme_apply_for);
    echo json_encode($data);
  }
