...
class Adm_phd_mis_registration extends MY_Controller
{
                ...
                if($login_id && $admn_type=='mtech')
                {
                 ...
                 // either print form or load form
                  ...
                  if($this->Phd_mis_reg->check($admn_no) && $tab5 != '')// already submitted
                  {
                     ...
                     //$this->load->view('new_admission_common/download_format_common',$details);// require for each admn_type
                  ...
                  else
                  {
                    ...
                    # comment receipt part here because it will be generated dynamically #
                      ...
                      //echo 'entered_success';exit;
                      // echo '<pre>';
                      // print_r($login_id);
                      // echo '</pre>';
                      // exit;
                      $this->adm_phd_header($data);
                      $this->load->view('admission/phd/mis_registration_common_form_new', $login_id);
                      $this->adm_phd_footer();
                      }
                      else {
                    ...
                    //  if ($tab_status == '5') {
                    //   redirect('admission/phd/Adm_phd_mis_registration_other_details/preview_submit');
                    //  }
                }
   ...
   function mis_registration_save_personal_details(){
    ...
    if ($registration_no && ($login_type =='Phd')) {
          ...
          //$this->form_validation->set_rules('identification_mark', 'Extra Curricular Activities', 'required|callback_check_identificationmark');
          ...
          //$this->form_validation->set_rules('aadhar_no', 'Aadhar Number', 'required|callback_check_aadhar_no');
          ...
          if ($this->form_validation->run() == TRUE && empty($upload_error)) {
              ...
              if ($personal_details_insert_log == '') {
                ...
                redirect('admission/phd/Adm_phd_mis_registration/manage_personal_details');
              }
              else {
   ...
   function parent_bank_account_details(){
        ...
        $admn_type='';
            ...
            if($login_id && $admn_type=='mtech')
            {
             ...
             // either print form or load form
              ...
              if($this->Phd_mis_reg->check($admn_no))// already submitted
              {
                 //echo 'entered registration'; exit;
                 $details=$this->getFromTables($admn_no,$admn_type);// will be same
                 $details=array_merge($details,$data);
                 $this->session->unset_userdata($admn_no,$admn_type);
                 $this->session->set_userdata("admn_no",$admn_no); // for download pdf check
    ...
    function our_upload($admn_no,$admn_type,$registration_no){
        ...
        // exit;
        $photo_file_ext = pathinfo($_FILES["photo"]["name"], PATHINFO_EXTENSION);
        $admn_no_lower = strtolower($admn_no);
        $config=array();
        $config['upload_path']='./assets/admission/phd/'.$registration_no.'/';
        ...
        if($upload_photo)
        {
          ...
          $_POST['photo']=base_url().'assets/admission/phd/'.$registration_no.'/'.$upload_data['file_name'];
        }
        else
        {
        ...
        {
          $upload_data = $this->upload->data();
          $_POST['sign']=base_url().'assets/admission/phd/'.$registration_no.'/'.$upload_data['file_name'];
        }
        else{
   ...
   function check_aadhar_no(){
    if ($this->input->post('aadhar_no') === 'none') {
			$this->form_validation->set_message('check_aadhar_no', 'Aadhar No. is required');
			return FALSE;
		} else {
			...
			$this->form_validation->set_message('check_aadhar_no', 'Aadhar No. cannot contain special characters');
			return FALSE;
		 }
		}
   }
