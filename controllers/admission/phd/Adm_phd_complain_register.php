...
class Adm_phd_complain_register extends MY_Controller
{
	function __construct()
    {
        parent::__construct();
        $this->load->model('admission/phd/adm_phd_complain_model','',TRUE);
    ...
    public function index()
    {
        ...
        $this->load->view('admission/phd/adm_phd_complain_view',$viewdata);
        $this->adm_phd_footer();
    ...
    public function save()
    {
        ...
        if($this->form_validation->run() == FALSE)
        {
            ...
            $data['val']="H";
            $this->adm_phd_header($data);
            $this->load->view('admission/phd/adm_phd_complain_view',$data);
            $this->adm_phd_footer();
        ...
        else
        {
            ...
            if($email_k!= $email)
            {
                ...
                redirect('admission/phd/Adm_phd_complain_register');
            }
            ...
            {
                $getval= $this->adm_phd_complain_model->check_order_no($order_no,$email);
            ...
            if(empty($upload_error['photo_error']))
            {
                ...
                if($this->adm_phd_complain_model->insert_complain($data)) // if data inserted successfully
                {
                    $this->session->set_flashdata('data_name', $com_id);
                    redirect('admission/phd/Adm_phd_complain_register/complain_details');
                ...
                else
                {
                    ...
                    $this->adm_phd_header($data);
                    $this->load->view('admission/phd/adm_phd_complain_app',$data);
                    $this->adm_phd_footer();
            ...
            else
            {
                ...
                $this->load->view('admission/phd/adm_phd_complain_view',$payment_data);
            }
    ...
    public function complain_details()
    {
        ...
        $where['com_id'] = $val;
        $data = $this->adm_phd_complain_model->get_complain($where);
        $data['msg'] = "<div  style=color:green;>Complain created successfully ! Please track complain status regularly. <div>";
        $data['val']="H";
        $this->adm_phd_header($data);
        $this->load->view('admission/phd/adm_phd_complain_app',$data[0]);
        $this->adm_phd_footer();
    }
    ...
    public function complain_status()
    {
        ...
        $data['val']="H";
        $this->adm_phd_header($data);
        $this->load->view('admission/phd/adm_phd_complain_status',$viewdata);
        $this->adm_phd_footer();
    ...
    public function track_status()
    {
        ...
        if(!empty($order_no))
        {
            ...
            if($getval=='ok')
            {
                ...
                $result['data'] = $this->adm_phd_complain_model->get_complain($where);
                $data['val']="H";
                $this->adm_phd_header($data);
                $this->load->view('admission/phd/adm_phd_complain_status',$result);
                $this->adm_phd_footer();
            }
            else
            {
                $this->session->set_flashdata('message_error','Order No  does not match error !');
                redirect('admission/phd/Adm_phd_complain_register/track_status');
            }
        ...
        else
        {
            ...
            $data['val']="H";
            $this->adm_phd_header($data);
            $this->load->view('admission/phd/adm_phd_complain_status',$error_data);
            $this->adm_phd_footer();
    ...
    function our_upload($order_no,$email,$com_id)
    {
        ...
        $dstFile = '/var/www/html/assets/admission/phd/complain/';
        $config=array();
        $config['upload_path']='./assets/admission/phd/complain/'; //  student_db_upload
        ...
        $srcPath = '/disk2/virtualhost/admission/public_html/html/assets/admission/phd/complain/';
        $srcImg = '/disk2/virtualhost/admission/public_html/html/assets/admission/phd/complain/'.$ImgName;
    ...
    public function order_no_exist($order_no) {
        ...
        $is_exist = $this->adm_phd_complain_model->isOrdernoEmailExist($order_no);
    ...
    public function order_no_in_log($order_no)
    {
        ...
        $in_log = $this->adm_phd_complain_model->isOrdernoInLog($order_no);
