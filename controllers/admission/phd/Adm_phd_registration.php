...
class Adm_phd_registration extends MY_Controller {
  ...
  function index()
  {
    ...
    $this->load->view('admission/phd/adm_phd_registration_view',$data);
    $this->Adm_phd_footer();
  }
  ...
  public function get_registration()
  {
    ...
    $m_email=$this->Add_phd_registration_model->check_email_exist($email);

    if($m_email=='ok')
    {

      $this->session->set_flashdata('email_mobile','Registration already done using same Email !');
      redirect('admission/phd/Add_phd/Adm_phd_registration');
      // redirect('admission/Add_mba/adm_mba_registration');
      exit;
    }

    $m_mobile=$this->Add_phd_registration_model->check_mobile_exist($mobile);
    ...
    if ($this->form_validation->run() == true)
    {
      ...
      $maxid=$this->Add_phd_registration_model->get_registration_max_id();
      $nemax=$maxid+1;
      // $year=date("y");
      $year =22;
      ...
      if(true)
      {
        ...
        $link="https://admission.iitism.ac.in/index.php/admission/phd/Adm_phd_registration/verify_email/".$email_decode;
        ...
        $htmlContent .= '<h5>Thank you for registering on IIT(ISM) Ph.D. Admission - 2022-2023 portal Phase(II)</h5>';
        ...
        if($this->email->send())
        {
          ...
          $result=$this->Add_phd_registration_model->get_registration($values);
          $msgok=$this->Add_phd_registration_model->insert_email_log($emlog);
          $update_sendmail=$this->Add_phd_registration_model->time_of_email_send($upval,$email);
...
public function track_email($p)
{
  ...
  {
    $this->Add_phd_registration_model->set_track_email($val,$email);
  }
}
...
public function user_login()
{
 ...
 // $secret='6LchRQIaAAAAAA_4rHRoCbvszIVDQmwlpqZZebm5';  dev
  ...
  if ($this->form_validation->run() == true)
  {
    if($status['success'])
    {
      $check_registration_number=$this->Add_phd_registration_model->check_registration_no(trim($this->input->post('user_name')));
      ...
      else
      {
        ...
        else
        {
          // echo "loginnn";
          // exit;
          $login=$this->Add_phd_registration_model->login($data1);
          $val_reg=$this->Add_phd_registration_model->get_email_registration($data1);
