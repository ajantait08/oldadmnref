...
class Adm_phd_mis_registration_other_details extends MY_Controller
{
            ...
            $admn_type='';
                ...
                if($login_id && $admn_type=='mtech')
                {
                 ...
                 // either print form or load form
                  ...
                  if($this->Phd_mis_reg->check($admn_no))// already submitted
                  {
                     $login_id=$this->getFromTables($admn_no,$admn_type);// will be same
                     $login_id=array_merge($login_id,$data);
                      ...
                      if (empty($reg_tab_status)) {
                       ...
                       $tab_status = $reg_tab_status[0]['tab_status'];
                        ...
                        if ($tab_status == '3') {
                            ...
                            $this->load->view('admission/phd/mis_registration_common_form_other_imp_details', $login_id);
                            $this->adm_phd_footer();
                        }
                     ...
                     // either print form or load form
                      ...
                      else
                      {
                        ...
                        # comment receipt part here because it will be generated dynamically #
                          ...
                          #xxxxxxxxxxxxxxxxxxxxxxxxxx JRF
                          $login_id['education_details'] = $this->Phd_mis_reg->get_jrf_education_details($login_id['reg_id'],$login_id['admn_no']);

                          $login_id['photo_signature'] = $this->Phd_mis_reg->get_photo_signature_jrf($login_id['reg_id'],$login_id['admn_no']);
                          /* get data from previous form */
                          $login_id['get_personal_details'] = $this->Phd_mis_reg->get_personal_details($login_id['reg_id'],$login_id['admn_no']);
                          $login_id['get_education_details'] = $this->Phd_mis_reg->get_education_details($login_id['reg_id'],$login_id['admn_no']);
                          $login_id['get_prev_education_details'] = $this->Phd_mis_reg->get_prev_education_details($login_id['reg_id'],$login_id['admn_no']);
                          $login_id['get_prev_certificate_details'] = $this->Phd_mis_reg->get_prev_certificate_details($login_id['reg_id'],$login_id['admn_no']);
                          $login_id['get_parent_details'] = $this->Phd_mis_reg->get_parent_details($login_id['reg_id'],$login_id['admn_no']);
                          $login_id['get_hostel_details'] = $this->Phd_mis_reg->get_hostel_details($login_id['reg_id'],$login_id['admn_no']);
                          $login_id['get_fee_details'] = $this->Phd_mis_reg->get_fee_details($login_id['reg_id'],$login_id['admn_no']);
                          $login_id['get_student_account_details'] = $this->Phd_mis_reg->get_student_account_details($login_id['reg_id'],$login_id['admn_no']);
                      ...
                      if ($tab_status == '4') {
                        $this->adm_phd_header($data);
                        $this->load->view('admission/phd/mis_registration_common_form_preview', $login_id);
    ...
    function save_registrtaion_no_other_details(){
        ...
        if ($registration_no && ($login_type =='Phd')) {
          ...
          $this->form_validation->set_rules('laptop_make', 'Laptop login_id', 'required|callback_check_laptop_make');
          ...
          if($this->form_validation->run() == TRUE) {
            ...
            if (!empty($check_hostel_details_exists)) {
              ...
              if ($hostel_details_insert_log == '') {
                ...
                redirect('admission/phd/Adm_phd_mis_registration_other_details/manage_previous_other_details');
              }
              else {
            ...
            if (!empty($check_stu_account_details_exists)) {
              ...
              if ($save_hostel_info != '' &&  $save_account_details != '' && $save_fee_details != '') {
    ...
    function download_pdf($admn_no,$admn_type,$reg_id)
  {
    ...
    if($this->session->userdata('admn_no'))
    {
      ...
      if($admn_no_session==$admn_no)
      {
        ...
        $this->load->helper(array('dompdf', 'file'));
        $ff=$this->load->view('admission/phd/mis_registration_print_final_pdf',$details,TRUE);
  ...
  //   exit;
    ...
    if($this->session->userdata('registration_no'))
    {
      ...
      $this->load->helper(array('dompdf', 'file'));
      $path = '/disk2/virtualhost/admission/public_html/html/assets/admission/phd/'.$admn_no_lower;
      ...
      $html = $this->load->view('admission/phd/mis_registration_print_final_pdf', $details, TRUE);
      //echo $html; exit;
      $paper = '';
      $stream = FALSE;
     ...
     $file_to_save_reg = '/disk2/virtualhost/admission/public_html/html/assets/admission/phd/'.$admn_no_lower.'/Mis_registration_form_'.$admn_no_lower.'.pdf';
    ...
    function send_email($admn_no,$admn_type,$email,$reg_id)
    {
     ...
     // $mail->SMTPSecure = PHPMailer::ENCRYPTION_STARTTLS;
      ...
      $file_path = '/disk2/virtualhost/admission/public_html/html/assets/admission/phd/'.strtolower($admn_no).'/Mis_registration_form_'.strtolower($admn_no).'.pdf';
...
// for mba it is not compulsory field
  if($admn_type=='mba' || $admn_type=='jrf')
  {
      $i++;
        $stu_prev_education_details[$i]['admn_no'] = $login_id['admn_no'];
        $stu_prev_education_details[$i]['sno'] = $i+1;
...
/*
  ...
  /*
  else if($admn_type=='mba')//department_id='ms'
  {
    $q=$login_id['course_id'].'_'.$login_id['branch_id'];
    ...
    // exit;
        ...
        //  exit;
         ...
         $admn_no = $login_id['admn_no'];
          ...
          $local_Path = '/disk2/virtualhost/admission/public_html/html/assets/admission/phd/'.$registration_id.'/';
          $local_Path_to_copy = '/disk2/virtualhost/admission/public_html/html/assets/admission/phd/'.$admn_no_lower.'/';
  ...
  public function download_receipt($receipt_payment)
    {
      ...
      $this->load->helper(array('dompdf', 'file'));
		  $path = '/var/www/html/assets/admission/phd/'.$receipt_payment['registration_no'];

		  $html = $this->load->view('admission/phd/generate_receipt_print_view', $data, TRUE);
     ...
     $file_to_save = FCPATH.'assets/admission/phd/'.$receipt_payment['registration_no'].'/Receipt__'.$receipt_payment['order_no'].'.pdf';
    ...
    //  }
     ...
     //exit;
      ...
      $data_receipt['receipt_path'] = 'assets/admission/phd/'.$receipt_payment['registration_no'].'/Receipt__'.$receipt_payment['order_no'].'.pdf';
