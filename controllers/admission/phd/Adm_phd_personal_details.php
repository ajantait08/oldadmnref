...
class Adm_phd_personal_details extends MY_Controller {
  ...
  { 
    ...
    $data['upload_document']=$this->Adm_phd_personal_details_model->get_all_document_uploaded($get_reg_no);
    ...
    $data['qualpg1_details']=$this->Adm_phd_personal_details_model->get_qualificationpg1_details($get_reg_no);
    $data['qualpg2_details']=$this->Adm_phd_personal_details_model->get_qualificationpg2_details($get_reg_no);
    ...
    $data['phd_fello_details1']=$this->Adm_phd_personal_details_model->get_phd_fello_score_details_one($get_reg_no);
    $data['phd_fello_details2']=$this->Adm_phd_personal_details_model->get_phd_fello_score_details_two($get_reg_no);
    $data['phd_fello_details3']=$this->Adm_phd_personal_details_model->get_phd_fello_score_details_three($get_reg_no);
    $data['phd_fello_details4']=$this->Adm_phd_personal_details_model->get_phd_fello_score_details_four($get_reg_no);
    $data['phd_fello_details_iit']=$this->Adm_phd_personal_details_model->get_phd_fello_score_details_iit($get_reg_no);
    ...
    $data['full_partime']=$this->Add_phd_registration_model->get_candidate_type_by_email($email);
    
    //base on conditon of prj ,AI ,EXT, PT,fecth dropdown 
    $data['fellowship_type_list']=$this->Adm_phd_personal_details_model->get_fellowship_list($data['application_details_ms'][0]['assistance_type'],$get_reg_no);
    // echo "<pre>";
    // print_r($data);
    // exit;
  ...
  { 
    ...
    $email=$this->session->userdata('email');
    $full_partime=$this->Add_phd_registration_model->get_candidate_type_by_email($email);
    ...
    { 
      ...
      { 
        ...
        { 
          ...
          if($working_project=='Y')
          {
            $this->Adm_phd_personal_details_model->update_project_deatils_yes_no($project_details_yes,$get_reg_no);  
          }
          else
          {
            $this->Adm_phd_personal_details_model->update_project_deatils_yes_no($project_details_no,$get_reg_no);
        ...
        {
          $this->Adm_phd_personal_details_model->update_project_deatils_yes_no($part_time,$get_reg_no);
  ...
  public function get_education_detail()
  {
    ...
    $email=$this->session->userdata('email');
    $get_reg_no=$this->Add_phd_registration_model->get_registration_no_by_email($email);
    $full_part_time=$this->Add_phd_registration_model->get_candidate_type_by_email($email);
    $data['application_details_ms']=$this->Adm_phd_personal_details_model->get_adm_phd_appl_ms($get_reg_no);
         ...
         $cond=array(
          ...
          { 
            ...
            {  
               ...
               $have=$this->Adm_phd_personal_details_model->check_phd_fello_row($get_reg_no,'one');
               if($have)
               {
                 $this->Adm_phd_personal_details_model->update_phd_fello_details($fellowship_u1,$get_reg_no,'one');
               }
               else // else insert row if added
               {
                 $this->Adm_phd_personal_details_model->insert_phd_fello_details($fellowship1);
               }
          ...
          { 
               $have=$this->Adm_phd_personal_details_model->check_phd_fello_row($get_reg_no,'two');
               if($have)
               {
                $this->Adm_phd_personal_details_model->update_phd_fello_details($fellowship_u2,$get_reg_no,'two');
               }
               else // else insert row if added
               {
                 $this->Adm_phd_personal_details_model->insert_phd_fello_details($fellowship2);
               }
          ...
          { 
            $have=$this->Adm_phd_personal_details_model->check_phd_fello_row($get_reg_no,'three');
            if($have)
            {
              $this->Adm_phd_personal_details_model->update_phd_fello_details($fellowship_u3,$get_reg_no,'three');
            }
            else // else insert row if added
            {
              $this->Adm_phd_personal_details_model->insert_phd_fello_details($fellowship3);
            }
          ...
          { 
            $have=$this->Adm_phd_personal_details_model->check_phd_fello_row($get_reg_no,'four');
            if($have)
            {
              $this->Adm_phd_personal_details_model->update_phd_fello_details($fellowship_u4,$get_reg_no,'four');
            }
            else // else insert row if added
            {
              $this->Adm_phd_personal_details_model->insert_phd_fello_details($fellowship4);
            }
          ...
          if($btech_cgpa=='Y')
          {
            $have=$this->Adm_phd_personal_details_model->check_phd_fello_row($get_reg_no,'IIT Fellow');
            // echo $this->db->last_query();
            // exit;
            if($have)
            {
              $this->Adm_phd_personal_details_model->update_phd_fello_details($fellowship_u_iit,$get_reg_no,'IIT Fellow');
            }
            else // else insert row if added
            {
              $this->Adm_phd_personal_details_model->insert_phd_fello_details($fellowship_iit);
            }
          ...
          { 
            $have=$this->Adm_phd_personal_details_model->check_qualification_row($get_reg_no,'PG1 Exam');
            if($have)
            {
              $this->Adm_phd_personal_details_model->update_qualification($u_educations_detailspg1,$get_reg_no,'PG1 Exam');
            }
            else // else insert row if added
            {
              $this->Adm_phd_personal_details_model->insert_qualification($educations_detailspg1);
            }
          ...
          { 
            $have=$this->Adm_phd_personal_details_model->check_qualification_row($get_reg_no,'PG2 Exam');
            if($have)
            {
              $this->Adm_phd_personal_details_model->update_qualification($u_educations_detailspg2,$get_reg_no,'PG2 Exam');
            }
            else // else insert row if added
            {
              $this->Adm_phd_personal_details_model->insert_qualification($educations_detailspg2);
            }
    ...
    {  
      ...
      $data['fellowship_type_list']=$this->Adm_phd_personal_details_model->get_fellowship_list($data['application_details_ms'][0]['assistance_type']);
  ...
  { 
    if(!($this->session->has_userdata('email')))
    {
      redirect('phd/Add_phd/adm_phd_login');
    }
