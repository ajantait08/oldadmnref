...
class Adm_phd_forgetp extends MY_Controller {
	...
	function __construct() 
  {
    ...
    $this->load->model('admission/phd/Adm_phd_application_preview_model', 'Mba_all', TRUE);
  }
  ...
  {     
    ...
    $this->load->view('admission/phd/adm_phd_forgetp_view',$data);
  }
...
{ 
  ...
  $this->load->model('admission/phd/Add_phd_registration_model');
  $tot_forget=$this->Add_phd_registration_model->total_forget_count($forget_email);
  ...
  if(count($tot_forget)>=4)
  {
    ...
    redirect('admission/phd/Adm_phd_forgetp');
    exit;
  }
 
  $check_reg=$this->Add_phd_registration_model->get_verify_email($forget_email); //check first email is verfied or not

  $check_mail_id=$this->Add_phd_registration_model->check_email_mobile_exist($forget_email,$forget_registration);
  ...
  { 
    ...
    { 
      $password=$this->Add_phd_registration_model->get_password_by_email($forget_email);
      $config = array(
        'protocol'  => 'smtp',
      ...
      $htmlContent .= '<h5>Recovery password for ph.d Admission - 2022-2023 portal</h5>';
      ...
      $this->email->subject('Password Recovery for Ph.d Admission - 2022-2023 portal');
      ...
      if($this->email->send())
      {
        ...
        $msgok=$this->Add_phd_registration_model->insert_email_log($upval);
        if($msgok=='ok')
        { 
