...
class Adm_mtech_complain_register extends MY_Controller
{
	function __construct()
    {
        parent::__construct();
        $this->load->model('admission/mtech/adm_mtech_complain_model','',TRUE);
    ...
    {   
        ...
        $this->load->view('admission/mtech/adm_mtech_complain_view',$viewdata);
    ...
    {    
        ...
        if($this->form_validation->run() == FALSE)
        {
            ...
            $data['val']="H";
            $this->adm_mtech_header($data);
            $this->load->view('admission/mtech/adm_mtech_complain_view',$data);
            $this->adm_mtech_footer();
        ...
        else
        {
            ...
            if($email_k!= $email)
            {
                ...
                redirect('admission/mtech/Adm_mtech_complain_register');
            }
            ...
            {
                $getval= $this->adm_mtech_complain_model->check_order_no($order_no,$email);
            ...
            if(empty($upload_error['photo_error']))
            {
                ...
                if($this->adm_mtech_complain_model->insert_complain($data)) // if data inserted successfully
                {
                    $this->session->set_flashdata('data_name', $com_id);
                    redirect('admission/mtech/Adm_mtech_complain_register/complain_details');
                ...
                else
                {
                    ...
                    $this->adm_mtech_header($data);
                    $this->load->view('admission/mtech/adm_mtech_complain_app',$data);
                    $this->adm_mtech_footer();
            ...
            else
            {
                ...
                $this->load->view('admission/mtech/adm_mtech_complain_view',$payment_data);
            }
    ...
    {   
        ...
        $where['com_id'] = $val;
        $data = $this->adm_mtech_complain_model->get_complain($where);
        $data['msg'] = "<div  style=color:green;>Complain created successfully ! Please track complain status regularly. <div>";
        $data['val']="H";
        $this->adm_mtech_header($data);
        $this->load->view('admission/mtech/adm_mtech_complain_app',$data[0]);
        $this->adm_mtech_footer();
    }
    ...
    {   
        ...
        $data['val']="H";
        $this->adm_mtech_header($data);
        $this->load->view('admission/mtech/adm_mtech_complain_status',$viewdata);
        $this->adm_mtech_footer();
    ...
    {  
        ...
        if(!empty($order_no))
        {   
            $getval= $this->adm_mtech_complain_model->check_order_no($order_no,$email_k);
                // echo $this->db->last_query();
                // exit;
            if($getval=='ok')
            {
                $where['ref_no'] = $order_no;
                $result['data'] = $this->adm_mtech_complain_model->get_complain($where);
                $data['val']="H";
                $this->adm_mtech_header($data);
                $this->load->view('admission/mtech/adm_mtech_complain_status',$result);
                $this->adm_mtech_footer();
            }
            else 
            {
                $this->session->set_flashdata('message_error','Order No  does not match error !');
                redirect('admission/mtech/Adm_mtech_complain_register/track_status');
            }
        ...
        else
        {
            ...
            $data['val']="H";
            $this->adm_mtech_header($data);
            $this->load->view('admission/mtech/adm_mtech_complain_status',$error_data);
            $this->adm_mtech_footer();
    ...
    {  
        ...
        //$dstFile = '/var/www/html/assets/admission/mtech/complain/';
        // connection (mis)
        ...
        $config=array();
        $config['upload_path']='./assets/admission/mtech/complain/'; //  student_db_upload
        ...
        $srcPath = '/var/www/html/assets/admission/mtech/complain/'.$ImgName;
        $srcImg = '/var/www/html/assets/admission/mtech/complain/'.$ImgName;
    ...
    public function order_no_exist($order_no) {
        ...
        $is_exist = $this->adm_mtech_complain_model->isOrdernoEmailExist($order_no);
    ...
    {   
        ...
        $in_log = $this->adm_mtech_complain_model->isOrdernoInLog($order_no);
