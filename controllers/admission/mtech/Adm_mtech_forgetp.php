...
class Adm_mtech_forgetp extends MY_Controller {
	...
	function __construct() 
  {
    ...
    $this->load->model('admission/mtech/Adm_mtech_application_preview_model', 'Mba_all', TRUE);
  }
  ...
  {     
    ...
    $this->load->view('admission/mtech/adm_mtech_forgetp_view',$data);
  }
...
{ 
  ...
  $this->load->model('admission/mtecha/Add_mtech_registration_model');
  $tot_forget=$this->Add_mtech_registration_model->total_forget_count($forget_email);
  ...
  if(count($tot_forget)>=4)
  {
    ...
    redirect('admission/mtech/Adm_mtech_forgetp');
    exit;
  }
 
  $check_reg=$this->Add_mtech_registration_model->get_verify_email($forget_email); //check first email is verfied or not

  $check_mail_id=$this->Add_mtech_registration_model->check_email_mobile_exist($forget_email,$forget_registration);
  ...
  { 
    ...
    { 
      $password=$this->Add_mtech_registration_model->get_password_by_email($forget_email);
      $config = array(
        'protocol'  => 'smtp',
        'smtp_host' => 'ssl://smtp.googlemail.com',
        'smtp_port' => 465,
        'smtp_user' => 'noreply.admission@iitism.ac.in',
      ...
      $this->email->from('noreply.admission@iitism.ac.in');
      $this->email->subject('Password Recovery for M.tech Admission - 2022-2023 portal');
      ...
      if($this->email->send())
      {
        ...
        $msgok=$this->Add_mtech_registration_model->insert_email_log($upval);
        if($msgok=='ok')
        { 
