...
class Adm_mtech_document extends MY_Controller {
  ...
  { 
    ...
    // $data['gate_score']=$total_document->gate_paper_code;
    // $data['pwd']=$total_document->pwd;
    // $data['color_blind']= $total_document->color_blind;
    // $data['work_experince']= $total_document->work_expreince;
    // $data['category']=$total_document->category;
    if($total_document[0]->work_expreince =='Yes')
    {
      $count=$count+1;
    }
    if(!empty($total_document[0]->gate_paper_code))
    {
      $count=$count+1;
    }
    ...
    if($total_document[0]->color_blind=='Y')
    {
      $count=$count+1;
    }
  ...
  public function document_submit()
  {
    ...
    { 
      ...
      {
        $this->session->set_flashdata('error','Error in coap registration internal data');
  ...
  public function validate_photo_signature_doc()
  {
    ...
    //$value=$this->Add_mtech_registration_model->get_application_programme_details($get_reg_no,$doc_id);
