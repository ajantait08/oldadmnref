...
class Adm_phdef_complain_register extends MY_Controller
{
	function __construct()
    {
        parent::__construct();
        $this->load->model('admission/phdef/adm_phdef_complain_model','',TRUE);
        ...
        $this->load->model('admission/phdef/Adm_phdef_personal_details_model');
          ...
          define('ftp_host','103.15.228.70'); // address of online.iitism.ac.in // making the connection from admision to mislive //172.16.200.88
    ...
    public function index()
    {
        ...
        $this->load->view('admission/phdef/adm_phdef_complain_view',$viewdata);
    ...
    public function save()
    {
        ...
        if($this->form_validation->run() == FALSE)
        {
            ...
            $data['val']="H";
            $this->adm_phdef_header($data);
            $this->load->view('admission/phdef/adm_phdef_complain_view',$data);
            $this->adm_phdef_footer();
        ...
        else
        {
            ...
            if($email_k!= $email)
            {
                ...
                redirect('admission/phdef/Adm_phdef_complain_register');
            }
            ...
            {
                $getval= $this->adm_phdef_complain_model->check_order_no($order_no,$email);
            ...
            if(empty($upload_error['photo_error']))
            {
                ...
                if($this->adm_phdef_complain_model->insert_complain($data)) // if data inserted successfully
                {
                    $this->session->set_flashdata('data_name', $com_id);
                    redirect('admission/phdef/Adm_phdef_complain_register/complain_details');
                ...
                else
                {
                    ...
                    $this->adm_phdef_header($data);
                    $this->load->view('admission/phdef/adm_phdef_complain_app',$data);
                    $this->adm_phdef_footer();
            ...
            else
            {
                ...
                $this->load->view('admission/phdef/adm_phdef_complain_view',$payment_data);
            }
    ...
    public function complain_details()
    {
        ...
        $where['com_id'] = $val;
        $data = $this->adm_phdef_complain_model->get_complain($where);
        $data['msg'] = "<div  style=color:green;>Complain created successfully ! Please track complain status regularly. <div>";
        $data['val']="H";
        $this->adm_phdef_header($data);
        $this->load->view('admission/phdef/adm_phdef_complain_app',$data[0]);
        $this->adm_phdef_footer();
    }
    ...
    public function complain_status()
    {
        ...
        $data['val']="H";
        $this->adm_phdef_header($data);
        $this->load->view('admission/phdef/adm_phdef_complain_status',$viewdata);
        $this->adm_phdef_footer();
    ...
    public function track_status()
    {
        ...
        if(!empty($order_no))
        {
            $getval= $this->adm_phdef_complain_model->check_order_no($order_no,$email_k);
                // echo $this->db->last_query();
                // exit;
            if($getval=='ok')
            {
                $where['ref_no'] = $order_no;
                $result['data'] = $this->adm_phdef_complain_model->get_complain($where);
                $data['val']="H";
                $this->adm_phdef_header($data);
                $this->load->view('admission/phdef/adm_phdef_complain_status',$result);
                $this->adm_phdef_footer();
            }
            else
            {
                $this->session->set_flashdata('message_error','Order No  does not match error !');
                redirect('admission/phdef/Adm_phdef_complain_register/track_status');
            }
        ...
        else
        {
            ...
            $data['val']="H";
            $this->adm_phdef_header($data);
            $this->load->view('admission/phdef/adm_phdef_complain_status',$error_data);
            $this->adm_phdef_footer();
    ...
    function our_upload($order_no,$email,$com_id)
    {
        ...
        // $connection2 = ssh2_connect('103.15.228.70', 22); //172.16.200.88
        ...
        // $sftp2 = ssh2_sftp($connection2);
        // $dstFile = '/var/www/html/assets/admission/ma/complain/';



        $config=array();
        $config['upload_path']='./assets/admission/phdef/complain/'; //  student_db_upload
        $config['overwrite'] = TRUE;
        //$config['allowed_types']='jpg|png|jpeg|JPG|pdf'; //
		$config['allowed_types']='*';
        $config['max_size']='1024';// in MB
        ...
        {
         //echo 'upload_complain_img_success'; exit;
        ...
        // $srcPath = '/disk2/virtualhost/admission/public_html/html/assets/admission/phdef/complain/';
        // $srcImg = '/disk2/virtualhost/admission/public_html/html/assets/admission/phdef/complain/'.$ImgName;
        // $newDstImg = $dstFile.$ImgName;
		// if(ssh2_scp_send($connection2, $srcImg, $newDstImg, 0664)){
        //     //echo 'file send';
        // }
        // else {
        //     //echo 'file cannot be send';
        // }
        // //exit;
        ...
        {
            //echo 'upload_complain_img_error';
    ...
    public function order_no_exist($order_no) {
        ...
        $is_exist = $this->adm_phdef_complain_model->isOrdernoEmailExist($order_no);
    ...
    public function order_no_in_log($order_no)
    {
        ...
        $in_log = $this->adm_phdef_complain_model->isOrdernoInLog($order_no);
