...
class Adm_phdef_document extends MY_Controller {
  ...
  function index()
  {
    ...
    $data['VIS']=$this->amd->check_fellowship_record($get_reg_no,'VIS-PhaseII');
    if(!empty($data['VIS']))
    {
      $count=$count+1;
    }
    ...
    $this->load->view('admission/phdef/adm_phdef_document_view',$data);
  ...
  public function document_upload($v)
  {
    ...
    $dstFile = '/var/www/html/assets/admission/phdef/'.$get_reg_no.'/';
    ...
    if (!file_exists('/disk2/virtualhost/admission/public_html/html/assets/admission/phdef/'.$get_reg_no))
    {
      mkdir('/disk2/virtualhost/admission/public_html/html/assets/admission/phdef/'.$get_reg_no, 0777, true);
    }


    // if (!file_exists('/home/admissiondev/public_html/admission/assets/admission/phdef/'.$get_reg_no))
    // {
    //   mkdir('/home/admissiondev/public_html/admission/assets/admission/phdef/'.$get_reg_no, 0777, true);
    // }
           ...
           else
            {
                ...
                else
                {
                  ...
                  else
                  {
                    ...
                    $srcImg = '/disk2/virtualhost/admission/public_html/html/assets/admission/phdef/'.$get_reg_no.'/'.$ImgName;
                    // $srcImg = '/var/www/html/assets/admission/ma/'.$get_reg_no.'/'.$ImgName;
                   ...
                   $cond=ssh2_scp_send($connection2, $srcImg, $newDstImg, 0777);
                    ...
                    $k=base_url()."assets/admission/phdef/".$get_reg_no."/".$file.$tran_id.$this->clean($_FILES[$file]['name']);
                    if($file=='photo'|| $file =='signature')
                    {
                      ...
                      $document=array(
                        ...
                        'doc_id'=>$doctype,
                        'doc_path'=>'assets/admission/phdef/'.$get_reg_no.'/'.$file.$tran_id.$this->clean($_FILES[$file]['name']),
                        'created_by'=>$email
                      );
                    }
                    else
  ...
  public function validate_photo_signature_doc()
  {
      ...
      // $vis=$this->amd->count_document_id($get_reg_no,'VIS-PhaseII');
      // if($vis=='ok')
      // {
      //   $count=$count+1;
      // }
