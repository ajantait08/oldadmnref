...
class Adm_phdef_admisison_payment extends MY_Controller {
	...
	function __construct()
  {
    ...
    $this->load->model('admission/phdef/Adm_phdef_admission_payment_model');
  ...
  function index()
  {
    ...
    $data['personal_details']=$this->Adm_phdef_admission_payment_model->get_adm_phdef_appl_ms($get_reg_no);
    ...
    $this->load->view('admission/phdef/adm_phdef_admisison_payment_view',$data);
  ...
  public function adm_phdef_payment()
  {
    ...
    $data['personal_details']=$this->Adm_phdef_admission_payment_model->get_adm_phdef_appl_ms_selected($get_reg_no,$data['program_selected']);
    // echo '<pre>';
    ...
    $data['program_apply']=$this->Adm_phdef_admission_payment_model->get_adm_phdef_reg_appl_program_selected($get_reg_no,$data['program_selected']);
    // echo '<pre>';
    // print_r($data);
    // echo '</pre>';
    // exit;
  ...
  public function proceed_to_payment()
  {
    ...
    $purpose = 'phdef Program Admission apply Fee';
    if($paygateway=='Sbi')
    {
      $order_no=$this->generate_order_no('pf','ISMS');
    }
    else if($paygateway=='Hdfc')
    {
      $order_no=$this->generate_order_no('pf','ISMH');
    }
    else
    {
    ...
    */
    ...
    $response_url = base_url('index.php/admission/phdef/Adm_phdef_admisison_payment/pay_response');
    ...
    $array_otherdetails = array(
      ...
      'payment_type' => 'PHDEF ADMISSION FEE',
  ...
  $log_data = array(
     ...
     $this->Adm_phdef_admission_payment_model->insert_payment_log_data($log_data); // insert in log table
  ...
  public function pay_response(){
    ...
    if(!empty($response_data))
    {
        ...
        if($bank_data['payment_status']=='FAIL' || $bank_data['payment_status']=='FAILURE')
            {
                $update_array = array(
                  ...
                  $update_log_fail = $this->Adm_phdef_admission_payment_model->update_main_payment_log($update_array,$where_data);
                  if ($update_log_fail) {
            ...
            else if($bank_data['payment_status']=="PENDING" || $bank_data['payment_status'] == "ABORTED")
            {
                $update_array = array(
                  ...
                  $update_log_pending = $this->Adm_phdef_admission_payment_model->update_main_payment_log($update_array,$where_data);
            ...
            else if($bank_data['payment_status']=="SUCCESS")
            {
                ...
                $update_log_success = $this->Adm_phdef_admission_payment_model->update_main_payment_log($update_array,$where_data);
                ...
                $tab_update=array(
                  'remark_1'=>"phdef Admission Fee done",

                );

                 $update_payment_tab_details = $this->Adm_phdef_admission_payment_model->update_payment_tab_details($tab_update, $where_data);
                 /* update payment in tab details */
                ...
                $update_payment_appl_ms_details = $this->Adm_phdef_admission_payment_model->update_payment_appl_ms_details($adm_phd_appl_ms, $where_data,$otherdetails_data['admn_type']);
      ...
      else
      {
            ...
            redirect('admission/phdef/Adm_phdef_admission_payment');
      }
  }
