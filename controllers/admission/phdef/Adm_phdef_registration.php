...
class Adm_phdef_registration extends MY_Controller
{
  ...
  public function get_registration()
  {
    ...
    $m_email = $this->Add_phdef_registration_model->check_email_exist($email);

    if ($m_email == 'ok') {

      $this->session->set_flashdata('email_mobile', 'Registration already done using same Email !');
      redirect('admission/phdef/Add_phdef/Adm_phdef_registration');
    ...
    if ($c_blind == '') {
      ...
      $this->session->set_flashdata('email_mobile', 'Color Blindness/Uniocularity field is mandatory!');
      redirect('admission/phdef/Add_phdef/Adm_phdef_registration');
      // redirect('admission/Add_mba/adm_mba_registration');
      exit;
    }

    $m_mobile = $this->Add_phdef_registration_model->check_mobile_exist($mobile);
    ...
    if ($this->form_validation->run() == true) {
      ...
      $maxid = $this->Add_phdef_registration_model->get_registration_max_id();
      $nemax = $maxid + 1;
      $year = date("y");
      // $year = 22;
      ...
      if (true) {
        ...
        $link = "https://admission.iitism.ac.in/index.php/admission/phdef/Adm_phdef_registration/verify_email/" . $email_decode;
        // $link = "https://admissiondev.iitism.ac.in/index.php/admission/phdef/Adm_phdef_registration/verify_email/" . $email_decode;
        ...
        $htmlContent .= '<h5>Thank you for registering on IIT(ISM) Ph.D. Admission (Externally Funded) </h5>';
        ...
        if ($this->email->send()) {
          ...
          $result = $this->Add_phdef_registration_model->get_registration($values);
          $msgok = $this->Add_phdef_registration_model->insert_email_log($emlog);
          $update_sendmail = $this->Add_phdef_registration_model->time_of_email_send($upval, $email);
  ...
  public function verify_email($mdcode)
  {
    ...
    $check = $this->Add_phdef_registration_model->get_verify_email($this->cleanspecailcharacter($decodegamil));
    $check_mail_id = $this->Add_phdef_registration_model->check_email_exist($this->cleanspecailcharacter($decodegamil));
    if ($check_mail_id == "not") {
      redirect('admission/phdef/Adm_phdef_registration/login_view');
      return false;
    }
    if ($check == 'Y') {
      $this->load->view('admission/phdef/adm_phdef_error_page');
    } else {
      ...
      if ($check_mail_id == 'ok') {
        ...
        $update_sendmail = $this->Add_phdef_registration_model->verify_email_time($vupval, $decodegamil);
  ...
  public function track_email($p)
  {
    ...
    if (md5($email_decode) == $p) {
      $this->Add_phdef_registration_model->set_track_email($val, $email);
    }
  }
  ...
  public function user_login()
  {
    ...
    // $this->load->model('admission/phd/Add_phd_registration_model', 'Add_phd_registration_model');
    ...
    $secret = '6Lfgv28kAAAAAIibPLOGEDnrRs0xb2XUSdr5vMdh';       //admissiondev
    // $secret = '6LdRAokdAAAAAOSeAQSJvzZ15st7BK-g3l7ZAI3V'; //admissionlive
      ...
      if (true) {
        ...
        if (empty($check_registration_number)) {
          ...
          if ($check_registration_number[0]->verification == '') {
            ...
            // echo "loginnn";
            // exit;
            $login = $this->Add_phdef_registration_model->login($data1);
            $val_reg = $this->Add_phdef_registration_model->get_email_registration($data1);
            ...
            if ($login) {
              ...
              $info['application'] = $this->Phdef_all->get_application_details($registration_no);
              // echo 'session';
              // echo '<pre>';
