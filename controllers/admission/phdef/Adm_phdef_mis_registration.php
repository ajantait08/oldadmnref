...
class Adm_phdef_mis_registration extends MY_Controller
{
            ...
            // $admn_type='';
                ...
                if($login_id && $admn_type=='jrf')
                {
                 ...
                 // either print form or load form
                  ...
                  //  echo $this->db->last_query().'<br>';
                  //  echo $pay_status_selected.'pay_status_selected<br>';
                  //  echo $tab4.'tab4<br>';
                  //  echo $pay_status_selected===false;
                  //  echo $tab4===false;
                  //  exit;

                  if($tab4 !='' && $pay_status_selected !='')// already submitted
                  // if($tab4 !='' && $pay_status_selected ===false)// already submitted
                  {
                  ...
                  else
                  {
                    ...
                    # comment receipt part here because it will be generated dynamically #
                      ...
                      $insert_tab_data = array(
                        ...
                        //'admn_no' => $login_id['admn_no'],
                        'apply_status' => 'phdef_mis_reg_details',
                        'created_by' => $registration_no,
                        'updated_date_time' => date('d-m-Y H:i:s'),
                        'tab_status' => 0
                      );
                      $init_tab_insert_status = $this->Phdef_mis_reg->insert_initial_tab_data($insert_tab_data);
                      if ($init_tab_insert_status) {
                      #echo 'entered success'; exit;
                      $this->adm_phdef_header($data);
                      $this->load->view('admission/phdef/mis_registration_common_form_new', $login_id);
                     ...
                     if ($tab_status == '5') {
                      redirect('admission/phdef/Adm_phdef_application_preview/admission_offer_letter');
                     }
                }
              }
                // else {
    ...
    function manage_personal_details(){
      ...
      if ($registration_no && ($login_type =='Phdef')) {
          ...
          $appl_type = $this->Phdef_mis_reg->get_appl_type_from_reg_no($registration_no);
          $login_id=$this->Phdef_mis_reg->validate_login($registration_no,$data['selected_program'],$appl_type);
          #$admission_no=$this->Phd_mis_reg->get_admn_no_from_stud_final_with_fee($registration_no);
          $admn_type='';
              if($login_id){
               $admn_type=$login_id['admn_type'];
               ...
               // either print form or load form
                ...
                else
                {
                  ...
                  # comment receipt part here because it will be generated dynamically #
                    ...
                    $login_id['get_prev_personal_details'] = $this->Phdef_mis_reg->get_prev_personal_details($login_id['reg_id']);
   ...
   function mis_registration_save_personal_details(){
    ...
    if ($registration_no && ($login_type =='Phdef')) {
          ...
          if ($this->form_validation->run() == TRUE && empty($upload_error)) {
              ...
              $check_personal_details_already_exists = $this->Phdef_mis_reg->check_personal_details_already_exists($registration_no);
              ...
              $personal_details_insert_log = $this->Phdef_mis_reg->insert_personal_details_log($data_insert_log);
              if ($personal_details_insert_log == '') {
                ...
                redirect('admission/phdef/Adm_phdef_mis_registration/manage_personal_details');
              }
              else {
                $delete_prev = $this->Phdef_mis_reg->delete_prev_personal_details($registration_no);
                if($delete_prev == ''){
                  $this->session->set_flashdata('flashError','Error Occured , Please Wait while trying to update personal details');
                  redirect('admission/phdef/Adm_phdef_mis_registration/manage_personal_details');
                }
              ...
              // echo '</pre>';
              // exit;
              $personal_details_insert = $this->Phdef_mis_reg->insert_personal_details($data_insert);
   ...
   function parent_bank_account_details(){
        ...
        $admn_type='';
            ...
            if($login_id && $admn_type=='jrf')
            {
             ...
             // either print form or load form
               ...
               $pay_status_selected = $this->Phdef_mis_reg->check_payment_status_appl_selected($registration_no);
               $tab4 = $this->Phdef_mis_reg->check_tab4_status($registration_no);
              if($this->Phdef_mis_reg->check($admn_no) && $tab4 != '')// already submitted
              {
                redirect('admission/phdef/Adm_phdef_application_preview');
              }
              else
    ...
    function our_upload($admn_no,$admn_type,$registration_no){
        ...
        $config=array();
        $config['upload_path']='./assets/admission/phdef/'.$registration_no.'/';
        ...
        if($upload_photo)
        {
          ...
          $_POST['photo']=base_url().'assets/admission/phdef/'.$registration_no.'/'.$upload_data['file_name'];
        }
        else
        {
        ...
        {
          $upload_data = $this->upload->data();
          $_POST['sign']=base_url().'assets/admission/phdef/'.$registration_no.'/'.$upload_data['file_name'];
        }
        else{
   ...
   function check_identificationmark(){
    if ($this->input->post('identification_mark') === 'none') {
			$this->form_validation->set_message('identification_mark', 'Identification Mark is required');
			return FALSE;
		} else {
			if(preg_match('/^[a-zA-Z0-9 \/_.]+$/', $this->input->post('identification_mark'))){				//echo 'entered_2';
				return TRUE;
		    }
		 else{
			$this->form_validation->set_message('identification_mark', 'Identification Mark cannot contain special characters');
			return FALSE;
		 }
		}
   }
