...
class Adm_mba_admin extends MY_Controller {
    ...
    $this->load->model('admission/Adm_mba_admin_model');
  }
  ...
  {  
    // echo "reach";
    // exit;
   $this->load->view('admission/adm_mba_admin_view');
  ...
  {   
    ...
    $this->load->model('admission/Adm_mba_admin_model');	  
    $login=$this->Adm_mba_admin_model->login($saltuser,$saltpass);
    // echo $this->db->last_query();
    // exit;
    if($login)
    { 
      $auth_type=$this->Adm_mba_admin_model->get_auth($saltuser,$saltpass);
     //   echo $this->db->last_query();
     //  exit;  
     // echo $auth_type;
      // exit;
      ...
      // exit;
      redirect('admission/Adm_mba_admin/dashboard');
       
    } 
    else
    {  
      $this->session->set_flashdata('fail','wrong user name and password');
    	redirect('admission/Adm_mba_admin');
 ...
 public function dashboard()
  { 
    ...
    if($var=='admin')
    { 
     $this->adm_mba_admin_header();
     $this->load->view('admission/adm_mba_admin_home_page');
     $this->adm_mba_admin_footer();
    }
   else
   {
    redirect('admission/Adm_mba_admin');
