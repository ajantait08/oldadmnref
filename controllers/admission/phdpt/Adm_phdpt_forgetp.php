...
class Adm_phdpt_forgetp extends MY_Controller {
	...
	function __construct() 
  {
    ...
    $this->load->model('admission/phdpt/Adm_phdpt_application_preview_model', 'Mba_all', TRUE);
  }
  ...
  {     
    ...
    $this->load->view('admission/phdpt/adm_phdpt_forgetp_view',$data);
...
{ 
  ...
  $this->load->model('admission/phdpt/Add_phdpt_registration_model');
  $tot_forget=$this->Add_phdpt_registration_model->total_forget_count($forget_email);
  ...
  if(count($tot_forget)>=4)
  {
    ...
    redirect('admission/phdpt/Adm_phdpt_forgetp');
    exit;
  }
 
  $check_reg=$this->Add_phdpt_registration_model->get_verify_email($forget_email); //check first email is verfied or not

  $check_mail_id=$this->Add_phdpt_registration_model->check_email_mobile_exist($forget_email,$forget_registration);
  ...
  { 
    ...
    { 
      $password=$this->Add_phdpt_registration_model->get_password_by_email($forget_email);
      $registration=$this->Add_phdpt_registration_model->get_registration_no_by_email($forget_email);
      $config = array(
        'protocol'  => 'smtp',
      ...
      $htmlContent .= '<h5>Recovery password for Ph.D. (Part Time) Admission - 2023-2024 portal</h5>';
      ...
      $this->email->subject('Password Recovery for Ph.D. (Part Time) Admission - 2023-2024 portal');
      ...
      if($this->email->send())
      {
        ...
        $msgok=$this->Add_phdpt_registration_model->insert_email_log($upval);
        if($msgok=='ok')
        { 
