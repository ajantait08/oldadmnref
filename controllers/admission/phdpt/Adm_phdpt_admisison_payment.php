...
class Adm_phdpt_admisison_payment extends MY_Controller {
	...
	function __construct()
  {
    ...
    $this->load->model('admission/phdpt/Adm_phdpt_admission_payment_model');
  ...
  function index()
  {
    ...
    $data['personal_details']=$this->Adm_phdpt_admission_payment_model->get_adm_phd_appl_ms($get_reg_no);
    $data['program_apply']=$this->Adm_phdpt_personal_details_model->get_adm_phd_reg_appl_program($get_reg_no);
    $data['count_amount']=count($data['program_apply']);
    $data['val']="H";
    $this->adm_phdpt_header($data);
    $this->load->view('admission/phdpt/adm_phdpt_admisison_payment_view',$data);
  ...
  public function adm_phd_payment()
  {
    ...
    $data['upload_document']=$this->Adm_phdpt_personal_details_model->get_all_document_uploaded($get_reg_no);
    $data['application_details']=$this->Add_phdpt_registration_model->get_application_programme_details($get_reg_no);
    //$data['personal_details']=$this->Adm_phd_admission_payment_model->get_adm_phd_appl_ms($get_reg_no);
    $data['personal_details']=$this->Adm_phdpt_admission_payment_model->get_adm_phd_appl_ms_selected($get_reg_no,$data['program_selected']);
    // echo '<pre>';
    // print_r($data['personal_details']);
    // echo '</pre>';
    // exit;
    //$data['program_apply']=$this->Adm_phd_personal_details_model->get_adm_phd_reg_appl_program($get_reg_no);
    $data['program_apply']=$this->Adm_phdpt_admission_payment_model->get_adm_phd_reg_appl_program_selected($get_reg_no,$data['program_selected']);
    // echo '<pre>';
    // print_r($data);
    // echo '</pre>';
    // exit;
    $data['order_no']=$this->generate_order_no('ADMphd','IIT');
    $data['count_amount']=count($data['program_apply']);
    $data['val']="H";
    $this->adm_phdpt_header($data);
    $this->load->view('admission/phdpt/adm_phdpt_admisison_payment_view',$data);
  ...
  function generate_order_no($admn_type,$iit)
  {
    ...
    $tran_id = substr($string_shuffled, 1, 3);
  ...
  public function proceed_to_payment()
  {
    ...
    if(!($this->session->has_userdata('login_type')))
    {
      redirect('admission/phdpt/Add_phdpt/adm_phd_login');
    }
    $this->load->model('admission/phdpt/Adm_phdpt_admission_payment_model');
    $this->load->model('admission/phdpt/Add_phdpt_registration_model');
    $email=$this->session->userdata('email');
    ...
    //$data['program_apply']=$this->Adm_phd_admission_payment_model->get_adm_phd_reg_appl_program($get_reg_no);
    $data['program_apply']=$this->Adm_phdpt_admission_payment_model->get_adm_phd_reg_appl_program_selected($get_reg_no,$data['program_selected']);
    ...
    $purpose = 'phdpt Program Admission apply Fee';
    if($paygateway=='Sbi')
    {
      $order_no=$this->generate_order_no('pt','ISMS');
    }
    else if($paygateway=='Hdfc')
    {
      $order_no=$this->generate_order_no('pt','ISMH');
    }
    else
    {
    ...
    // $data['fee_details'] = $this->Adm_phd_admission_payment_model->insert_mis_pay_adm_pay_detail($payment_data_insert,$condition);
    // echo '<pre>';
    ...
    if($paygateway=='Sbi')
    {
      ...
      ?>
      <script>
        window.location.href="https://pay.iitism.ac.in/index.php/Phd_fee_pay_adm/Adm_phd_fee/pay?q=" + '<?php echo urlencode($payment_data_encrypt);?>';
      </script>
      <?php
    }
    else if($paygateway=='Hdfc')
    {
      ...
      ?>
      <script>
        window.location.href="https://pay.iitism.ac.in/index.php/phdadmissionhdfcdev/Adm_phd_hdfc_fee/pay?q=" + '<?php echo urlencode($payment_data_encrypt);?>';
      </script>
      <?php
    }
    else
    ...
    */
    ...
    $response_url = base_url('index.php/admission/phdpt/Adm_phdpt_admisison_payment/pay_response');
    ...
    $array_otherdetails = array(
      ...
      'payment_type' => 'PHDPT ADMISSION FEE',
      ...
      'admn_type' => $data['personal_details'][0]['program_id'],
      'remark1' => 'NA',
      'module_name' => 'phdpt_admission_fee'
  );
  ...
  $log_data = array(
     ...
     $this->Adm_phdpt_admission_payment_model->insert_payment_log_data($log_data); // insert in log table
  ...
  public function pay_response(){
    ...
    if(!empty($response_data))
    {
        ...
        if($bank_data['payment_status']=='FAIL')
            {
                $update_array = array(
                  ...
                  $update_log_fail = $this->Adm_phdpt_admission_payment_model->update_main_payment_log($update_array,$where_data);
                  if ($update_log_fail) {
                  ...
                  $failure_final_array['receipt_payment'] = array(
                    ...
                    $this->adm_phdpt_header($data);
                    $this->load->view('admission/phdpt/receipt_payment_view',$failure_final_array);
                    $this->adm_phdpt_footer();
            ...
            else if($bank_data['payment_status']=="PENDING")
            {
                $update_array = array(
                  ...
                  $update_log_pending = $this->Adm_phdpt_admission_payment_model->update_main_payment_log($update_array,$where_data);
                  ...
                  $this->adm_phdpt_header($data);
                  $this->load->view('admission/phdpt/receipt_payment_view',$pending_final_array);
                  $this->adm_phdpt_footer();
            }
            ...
            else if($bank_data['payment_status']=="SUCCESS")
            {
                ...
                $update_log_success = $this->Adm_phdpt_admission_payment_model->update_main_payment_log($update_array,$where_data);
                ...
                $tab_update=array(
                  'remark_1'=>"phdpt Admission Fee done",

                );

                 $update_payment_tab_details = $this->Adm_phdpt_admission_payment_model->update_payment_tab_details($tab_update, $where_data);
                 /* update payment in tab details */
                ...
                $update_payment_appl_ms_details = $this->Adm_phdpt_admission_payment_model->update_payment_appl_ms_details($adm_phd_appl_ms, $where_data,$otherdetails_data['admn_type']);
                ...
                if($update_log_success && $update_payment_tab_details && $update_payment_appl_ms_details) {
                    ...
                    $this->adm_phdpt_header($data);
                    $this->load->view('admission/phdpt/receipt_payment_view',$success_final_array);
                    $this->adm_phdpt_footer();
      ...
      else
      {
            ...
            redirect('admission/phdpt/Adm_phdpt_admission_payment');
      }
  }
