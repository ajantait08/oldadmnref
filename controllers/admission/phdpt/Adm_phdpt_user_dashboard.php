...
class Adm_phdpt_user_dashboard extends MY_Controller {
  ...
  function index()
  {
    ...
    $data['gate_paper_code']=$this->Add_phdpt_registration_model->gate_paper_code();
    $data['btech_paper']=$this->Add_phdpt_registration_model->get_programme_list_of_btech();

    $data['candidate_type']=$this->Add_phdpt_registration_model->get_candidate_type_by_email($email);
    $appl_ms=$this->Adm_phdpt_personal_details_model->get_adm_phd_appl_ms($get_reg_no);
    $appl_program=$this->Adm_phdpt_personal_details_model->get_adm_phd_reg_appl_program($get_reg_no);
    ...
    if(empty($appl_ms))
    {
      ...
      $this->load->view('admission/phdpt/adm_phdpt_apply',$viewdata);
  ...
  public function applications_track()
  {
    ...
    if(empty($tab_status))
    {
      redirect('admission/phdpt/Adm_phdpt_user_dashboard/apply_post');
    }
    else
    {
      if ($value=='1')
  ...
  public function education_apply_post()//for deleting temp file
  {
    ...
    $p_elig_desc=$this->Add_phdpt_registration_model->get_degree_desc_by_program_id($prog_elligibilty);
    $p_apply_f_desc=$this->Add_phdpt_registration_model->get_program_desc_by_program_id($programme_apply_for);
    $get_row_id_mis = $this->Add_phdpt_registration_model->check_application_programme_details_row_id_mis($get_reg_no,$programme_apply_for);
    $val=array(
      'id'=>$row_id,
    ...
    $p_row_id=$this->Add_phdpt_registration_model->check_application_programme_details_row_id($get_reg_no,$programme_apply_for);
    if($p_row_id==$row_id)
    {
      $ok=$this->Add_phdpt_registration_model->delete_applying_program($val);
      $ok2=$this->Add_phdpt_registration_model->delete_applying_program_mis($valmis);
      $this->Add_phdpt_registration_model->delete_specialization($specialization);
  ...
  {
    //echo 'required enetred here !'; exit;
    $this->load->model('admission/phdpt/Add_phdpt_registration_model');
    $this->load->model('admission/phdpt/Adm_phdpt_application_model');
    $this->load->model('admission/phdpt/Adm_phdpt_personal_details_model');
    $email= $this->session->userdata('email');
    ...
    $get_reg_no=$this->Add_phdpt_registration_model->get_registration_no_by_email($email);
    $appl_ms_prom=$this->Adm_phdpt_application_model->get_application_fill_details($get_reg_no);
    $datat['candidate_type']=$this->Add_phdpt_registration_model->get_candidate_type_by_email($email);
    $datac['cfti_yes']=$this->Add_phdpt_registration_model->get_cfti_by_reg($get_reg_no);
    ...
    $this->Add_phdpt_registration_model->update_fee_amount($fee,$get_reg_no);
    $this->Adm_phdpt_personal_details_model->update_tab1($tab,$get_reg_no);
    $this->Adm_phdpt_personal_details_model->update_tab1($tabs,$get_reg_no);
    $this->session->set_userdata('personal_details','personal_details');
  ...
  public function apply_post()
  {
    ...
    if(!empty($get_reg_details[0]->color_blind))
    {
      ...
      {
        $data['btech_paper']=$this->Add_phdpt_registration_model->get_programme_list_of_btech_without_pwd();
      }

      if($get_reg_details[0]->color_blind=='Y' And $get_reg_details[0]->pwd=='N')
      {
        $data['btech_paper']=$this->Add_phdpt_registration_model->get_programme_list_of_btech_without_color_blind();
      }
      ...
      {
        $data['btech_paper']=$this->Add_phdpt_registration_model->get_programme_list_of_btech_without_pwds();
 // echo $this->db->last_query();
      }
    ...
    //  echo $get_reg_details[0]->color_blind; echo "<br>";
    //  echo $get_reg_details[0]->pwd; echo "<br>";
    // echo "<pre>";
    // print_r($get_reg_details);
    // print_r($data);
    // exit;
    ...
    $this->adm_phdpt_header($data);
    $this->load->view('admission/phdpt/adm_phdpt_apply_post',$data);
    $this->adm_phdpt_footer();
  ...
  public function get_apply_post()
  {
    ...
    if(empty($get_reg_details))
    {
      ...
      $data['val']="H";
      $this->adm_phdpt_header($data);
      $this->load->view('admission/phdpt/adm_phdpt_apply_post');
      $this->adm_phdpt_footer();
    }
     ...
     $phd_in= trim($this->input->post('phd_in'));
      ...
      $val_apply=$this->Adm_phdpt_application_model->already_applied($cond);
      $appl_program=$this->Adm_phdpt_personal_details_model->get_adm_phd_reg_appl_program($get_reg_no);
      // echo $this->db->last_query();
      // exit;
    ...
    $elligibilty_y_n=$this->Add_phdpt_registration_model->get_mining_non_mining($data);
    $p_elig_desc=$this->Add_phdpt_registration_model->get_degree_desc_by_program_id($prog_elligibilty);
    $p_apply_f_desc=$this->Add_phdpt_registration_model->get_program_desc_by_program_id($programme_apply_for);
    $department=$this->Add_phdpt_registration_model->get_department_by_program_id($programme_apply_for);
    ...
    if ($this->form_validation->run() == true)
    {
      ...
      $ok1=$this->Adm_phdpt_application_model->insert_application_details($values);
      $this->Adm_phdpt_application_model->insert_specailization($prio1);

      if(!empty($Priority2))
      {
        $this->Adm_phdpt_application_model->insert_specailization($prio2);
      }

      if(!empty($Priority3))
      {
        $this->Adm_phdpt_application_model->insert_specailization($prio3);
      }
      if(!empty($Priority4))
      {
        $this->Adm_phdpt_application_model->insert_specailization($prio4);
      }
      if(!empty($Priority5))
      {
        $this->Adm_phdpt_application_model->insert_specailization($prio5);
      }


      //  echo $this->db->last_query();
      $ok2=$this->Adm_phdpt_application_model->insert_application_programme($value_apl_prog);

      $ok3=$this->Adm_phdpt_application_model->insert_phd_tab($values1);
      // echo $this->db->last_query();
      //  echo $ok2;
      //  exit;
  ...
  public function get_check_sub_math_12th()
  {
    ...
    $sub_math_12th=$this->Add_phdpt_registration_model->sub_math_12th($programme_apply_for);
  ...
  public function get_program_elligibilty()
  {
    ...
    $this->load->model('admission/phdpt/Add_phdpt_registration_model');
    $programme_apply_for = $this->input->post('programme_apply_for');
    $dept_ids=$this->Add_phdpt_registration_model->get_department_id_by_program_id($programme_apply_for);
    $data['prog'] = $this->Add_phdpt_registration_model->program_phd($dept_ids);
    $data['elligibilty'] = $this->Add_phdpt_registration_model->program_elligibilty_by_programe($programme_apply_for);
  ...
  public function get_specialization_by_program_id()
  {
    $this->load->model('admission/phdpt/Adm_phdpt_personal_details_model');
    $programme_apply_for = $this->input->post('programme_apply_for');
   $data['specail'] = $this->Adm_phdpt_personal_details_model->get_specialization($programme_apply_for);
    echo json_encode($data);
  }
  ...
  public function get_faculty_by_dept_id()
  {
   ...
   $data['specail'] = $this->Adm_phdpt_personal_details_model->get_faculty($dept);
    echo json_encode($data);
  }
