...
class Adm_phdpt_payment extends MY_Controller {
	...
	function __construct()
  {
    ...
    $this->load->model('admission/phdpt/Adm_phdpt_payment_model');
  ...
  function index()
  {
    ...
    $data['personal_details']=$this->Adm_phdpt_payment_model->get_adm_phd_appl_ms($get_reg_no);
   ...
   $check_blank_field='';
   $apl_ms=$this->Adm_phdpt_personal_details_model->get_adm_phd_appl_ms($get_reg_no);
   ...
   $full_part_time_type=$this->Add_phdpt_registration_model->get_candidate_type_by_email($email);
   ...
   $qual12_details=$this->Adm_phdpt_personal_details_model->get_qualification12_details($get_reg_no);
   ...
   $qual10_details=$this->Adm_phdpt_personal_details_model->get_qualification10_details($get_reg_no);
   ...
   $qualug_details=$this->Adm_phdpt_personal_details_model->get_qualificationug_details($get_reg_no);
  ...
  //  }
   ...
   $p_addr_details_c=$this->Adm_phdpt_personal_details_model->get_p_address_details($get_reg_no);
   ...
   $c_addr_detailsc=$this->Adm_phdpt_personal_details_model->get_c_address_details($get_reg_no);
   ...
   $upload_document=$this->Adm_phdpt_personal_details_model->get_all_document_uploaded($get_reg_no);
  ...
  //  print_r($apl_ms);
   ...
   if($check_blank_field=='blank')
   {
    ...
    $this->load->view('admission/phdpt/adm_phdpt_error_found',$data_check);
   ...
   else
   {
     $data['val']="H";
     $this->adm_phdpt_header($data);
     $this->load->view('admission/phdpt/adm_phdpt_payment_view',$data);
     $this->adm_phdpt_footer();
   }
  ...
  public function adm_phdpt_payment()
  {
  ...
  public function proceed_to_payment()
  {
    ...
    $data['program_apply']=$this->Adm_phdpt_payment_model->get_adm_phd_reg_appl_program($get_reg_no);
    ...
    $purpose = 'phd PT Program apply Fee';
    if($paygateway=='Sbi')
    {
      $order_no=$this->generate_order_no('PHDPT','IITISMSBI');
    }
    else if($paygateway=='Hdfc')
    {
      $order_no=$this->generate_order_no('PHDPT','IITISMHDFC');
    }
    else
    {
    ...
    $response_url = base_url('index.php/admission/phdpt/Adm_phdpt_payment/pay_response');
    ...
    $array_otherdetails = array(
      ...
      'payment_type' => 'PHDPT APPLICATION FEE',
      ...
      'admn_type' => 'phdpt',
      'remark1' => 'NA',
      'module_name' => 'phdpt_application_fee'
  );
  ...
  $log_data = array(
     ...
     $this->Adm_phdpt_payment_model->insert_payment_log_data($log_data); // insert in log table
        ...
        if($bank_data['payment_status']=='FAIL')
            {
                $update_array = array(
                  ...
                  $update_log_fail = $this->Adm_phdpt_payment_model->update_main_payment_log($update_array,$where_data);
                  if ($update_log_fail) {
            ...
            else if($bank_data['payment_status']=="PENDING")
            {
                $update_array = array(
                  ...
                  $update_log_pending = $this->Adm_phdpt_payment_model->update_main_payment_log($update_array,$where_data);
            ...
            else if($bank_data['payment_status']=="SUCCESS")
            {
                ...
                $update_log_success = $this->Adm_phdpt_payment_model->update_main_payment_log($update_array,$where_data);
                ...
                $tab_update=array(
                 ...
                 $update_payment_tab_details = $this->Adm_phdpt_payment_model->update_payment_tab_details($tab_update, $where_data);
                 /* update payment in tab details */
                ...
                $update_payment_appl_ms_details = $this->Adm_phdpt_payment_model->update_payment_appl_ms_details($adm_phd_appl_ms, $where_data);
