...
class Adm_phdpt_complain_register extends MY_Controller
{
	function __construct()
    {
        parent::__construct();
        $this->load->model('admission/phdpt/adm_phdpt_complain_model','',TRUE);
        //$this->load->model('admission/phd/adm_phd_technical_complain_model','',TRUE);
        ...
        $this->load->model('admission/phdpt/Adm_phdpt_personal_details_model');
          ...
          define('ftp_host','172.16.200.88'); // address of online.iitism.ac.in // making the connection from admision to mislive
          // define('ftp_host','172.16.200.86'); // address of beta.iitism.ac.in  // making the connection from misdev and pbeta
           define('ftp_user_name','misadmin'); //from admision to mislive
           define('ftp_user_pass','qwerty!43@1'); //from admision to mislive
    ...
    public function index()
    {
        ...
        $tab_status=$this->Adm_phdpt_personal_details_model->get_tab_status($get_reg_no);
        $tab=$this->Adm_phdpt_personal_details_model->check_tab($get_reg_no);
        if(!empty($tab))
        {
        ...
        $this->load->view('admission/phdpt/adm_phdpt_complain_view',$viewdata);
        $this->adm_phdpt_footer();

    }

    public function technical_complaint(){
        $this->load->model('admission/phdpt/Adm_phdpt_personal_details_model');
        $email= $this->session->userdata('email');
        if(!($this->session->has_userdata('login_type')))
        {
        ...
        $data['complaint_list'] = $this->adm_phdpt_complain_model->get_complaint_type_list();

        $data['val']="H";
        $this->adm_phdpt_header($data);
        $this->load->view('admission/phdpt/adm_phdpt_complain_technical_view',$viewdata);
        $this->adm_phdpt_footer();
    ...
    public function save()
    {
        ...
        if($this->form_validation->run() == FALSE)
        {
            ...
            $data['val']="H";
            $this->adm_phdpt_header($data);
            $this->load->view('admission/phdpt/adm_phdpt_complain_view',$data);
            $this->adm_phdpt_footer();
        ...
        else
        {
            ...
            {
                $getval= $this->adm_phdpt_complain_model->check_order_no($order_no,$email);
            ...
            // exit;
            $upload_error=$this->our_upload($order_no,$email,$com_id);  // make it to return errors of file upload
            ...
            if(empty($upload_error['photo_error']))
            {
                ...
                if($this->adm_phdpt_complain_model->insert_complain($data)) // if data inserted successfully
                {
                    $this->session->set_flashdata('data_name', $com_id);
                    redirect('admission/phdpt/Adm_phdpt_complain_register/complain_details');
                ...
                else
                {
                    ...
                    $this->adm_phdpt_header($data);
                    $this->load->view('admission/phdpt/adm_phdpt_complain_app',$data);
                    $this->adm_phdpt_footer();
            ...
            else
            {
                ...
                $this->load->view('admission/phdpt/adm_phdpt_complain_view',$payment_data);
            }

        }
    }

    public function save_technical_complaint(){
        if(!($this->session->has_userdata('login_type')))
        {
           redirect('admission/phdpt/Add_phdpt/adm_phdpt_login');
        }
        $this->load->helper(array('form', 'url'));
        $this->load->library('form_validation');
        $this->form_validation->set_rules('order_no', 'Order Number', 'callback_order_no_exist');
        // $this->form_validation->set_rules('order_no', 'Order Number', 'required|callback_order_no_exist|callback_order_no_in_log');// is_unique[newadmission_complaint.ref_no]|
        $this->form_validation->set_rules('email', 'Email id', 'required');
        $this->form_validation->set_rules('contact_no', 'Contact No', 'required');
        $this->form_validation->set_rules('complain_type', 'Complain type', 'required');
        $this->form_validation->set_rules('application_type', 'Application type', 'required');
        $this->form_validation->set_rules('complain_details', 'Complain Details', 'required');
        ...
        else
        {
            ...
            $complain_type =$this->clean($this->input->post('complain_type'));
            $complain_details =$this->apha_numeric_only($this->input->post('complain_details'));
            $type =$this->clean($this->input->post('application_type'));
            $email_k= $this->session->userdata('email');
            // echo $email;
            // echo $email_k;
            // exit;
            if(trim($email_k) != trim($email))
            {
                //echo 'not match'; exit;
                $this->session->set_flashdata('message_error','Registered Email id does not match error !');
                redirect('admission/phdpt/Adm_phdpt_complain_register/technical_complaint');
            }
            if($order_no=="NA" || $order_no=="na")
            {
                //echo 'match'; exit;
            ...
            if(empty($upload_error['photo_error']))
            {
                ...
                if($this->adm_phdpt_complain_model->insert_technical_complain($data)) // if data inserted successfully
                {
                    $this->session->set_flashdata('data_name', $com_id);
                    redirect('admission/phdpt/Adm_phdpt_complain_register/complain_technical_details');
    ...
    public function complain_details()
    {
        ...
        $where['com_id'] = $val;
        $data = $this->adm_phdpt_complain_model->get_complain($where);
        $data['msg'] = "<div  style=color:green;>Complain created successfully ! Please track complain status regularly. <div>";
        $data['val']="H";
        $this->adm_phdpt_header($data);
        $this->load->view('admission/phdpt/adm_phdpt_complain_app',$data[0]);
        $this->adm_phdpt_footer();
    }

    public function complain_technical_details()
    {
        if(!($this->session->has_userdata('login_type')))
        {
           redirect('admission/phdpt/Add_phdpt/adm_phdpt_login');
        ...
        $where['com_id'] = $val;
        $data = $this->adm_phdpt_complain_model->get_technical_complain($where);
        $data['msg'] = "<div  style=color:green;>Complain created successfully ! Please track complain status regularly. <div>";
        $data['val']="H";
        $this->adm_phdpt_header($data);
        $this->load->view('admission/phdpt/adm_phdpt_technical_complain_app',$data[0]);
        $this->adm_phdpt_footer();
    }
    ...
    public function complain_status()
    {
        ...
        $data['val']="H";
        $this->adm_phdpt_header($data);
        $this->load->view('admission/phdpt/adm_phdpt_complain_status',$viewdata);
        $this->adm_phdpt_footer();
    ...
    public function track_status()
    {
        ...
        if(!empty($order_no))
        {
            $getval= $this->adm_phdpt_complain_model->check_order_no($order_no,$email_k);
                // echo $this->db->last_query();
                // exit;
            if($getval=='ok')
            {
                $where['ref_no'] = $order_no;
                $result['data'] = $this->adm_phdpt_complain_model->get_complain($where);
                $data['val']="H";
                $this->adm_phdpt_header($data);
                $this->load->view('admission/phdpt/adm_phdpt_complain_status',$result);
                $this->adm_phdpt_footer();
            }
            else
            {
                $this->session->set_flashdata('message_error','Order No  does not match error !');
                redirect('admission/phdpt/Adm_phdpt_complain_register/track_status');
            }
        ...
        else
        {
            ...
            $data['val']="H";
            $this->adm_phdpt_header($data);
            $this->load->view('admission/phdpt/adm_phdpt_complain_status',$error_data);
            $this->adm_phdpt_footer();

        }
    }

    public function track_technical_status()
    {
        if(!($this->session->has_userdata('login_type')))
        {
           redirect('admission/phdpt/Add_phdpt/adm_phdpt_login');
        ...
        $reg_k = $this->session->userdata('registration_no');
        $email = $this->input->post('email');
        $email_k= $this->session->userdata('email');
        if(!empty($registration_no))
        {
            // $getval= $this->adm_phd_complain_model->check_order_no($order_no,$email_k);
            //     // echo $this->db->last_query();
            //     // exit;
            if($registration_no === $reg_k)
            {
                $where['registration_no'] = $registration_no;
                $result['data'] = $this->adm_phdpt_complain_model->get_technical_complain($where);
                $data['val']="H";
                $this->adm_phdpt_header($data);
                $this->load->view('admission/phdpt/adm_phdpt_track_complain_status',$result);
                $this->adm_phdpt_footer();
            }
            else
            {
                $this->session->set_flashdata('message_error','Registration No. does not match error !');
                redirect('admission/phdpt/Adm_phdpt_complain_register/track_technical_status');
            }
        }
        else if(!empty($email))
        {
        ...
        {
            $error_data['msg'] = "<div style=color:red;>Field can not be blank !<div>";
            $data['registration_no'] = $this->session->userdata('registration_no');
            $data['val']="H";
            $this->adm_phdpt_header($data);
            $this->load->view('admission/phdpt/adm_phdpt_track_complain_status',$error_data);
            $this->adm_phdpt_footer();
    ...
    function our_upload($order_no,$email,$com_id)
    {
        ...
        //$dstFile = '/var/www/html/assets/admission/phd/complain/';
        // connection (mis)
        ...
        $sftp2 = ssh2_sftp($connection2);
        $dstFile = '/var/www/html/assets/admission/phdpt/complain/';

        $config=array();
        $config['upload_path']='/disk2/virtualhost/admission/public_html/html/assets/admission/phdpt/complain'; //  student_db_upload
        $config['overwrite'] = TRUE;
        //$config['allowed_types']='jpg|png|jpeg|JPG|pdf'; //
		$config['allowed_types']='*';
        $config['max_size']='20048';// in KB
        if ($order_no !== '') {
            $config['file_name'] = 'com_'.$com_id.'_'.$order_no;
        }
        else{
            $config['file_name'] = 'com_'.$com_id;
        }
        ...
        $srcPath = '/var/www/html/assets/admission/phdpt/complain/'.$ImgName;
        $srcImg = '/disk2/virtualhost/admission/public_html/html/assets/admission/phdpt/complain/'.$ImgName;
    ...
    public function order_no_exist($order_no) {
        ...
        $is_exist = $this->adm_phdpt_complain_model->isOrdernoEmailExist($order_no);
    ...
    public function order_no_in_log($order_no)
    {
        ...
        $in_log = $this->adm_phdpt_complain_model->isOrdernoInLog($order_no);
