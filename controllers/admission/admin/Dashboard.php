...
class Dashboard extends MY_Controller
{
  ...
  {
    $data['applications'] = $this->Mba_all->get_application_details_reg_asc();
   ...
   // exit;
     $this->load->view('admission/admin/layout/header');
     $this->load->view('admission/admin/show_candidate_list_doc',$data);
     $this->load->view('admission/admin/layout/footer');
  ...
  public function scrutiny_remark($registration_no)
  {
    ...
    $mba = $this->input->post('prog2');
    $tot_prom = $this->input->post('tot_pro');
    $program= $this->Mba_one->get_program_details($registration_no);

    // echo "<pre>";
    // $rejectReason = $this->input->post('reason_check');
    // $verf_remark=trim($this->input->post('verf_remark'));
    // echo $verf_action; echo "<br>";
    // echo $ba;echo "<br>";
    // echo $mba;echo "<br>";
    // echo $tot_prom;echo "<br>";
    // print_r($program);echo "<br>";
    // print_r($rejectReason);echo "<br>";
    // echo $verf_remark;
    // exit;
    ...
    else 
    {
      switch ($verf_action)
      {
        ...
        case 2:
          ...
          break;
        case 3:
          $verf_remark1 = "Partial Verified - (MBA OK) AND (BA NOT OK)";
          $mba_dec='V';
          $ba_dec='R';
          break;
        case 4:
          $verf_remark1 = "Partial Verified - (BA OK) AND (MBA NOT OK)";
          $mba_dec='R';
          $ba_dec='V';
          break;
        default:
    ...
    { 
      ...
      $value_anyone_status=$this->Mba_all->get_app_doc_verfied_status($wheresingle);
      ...
      $this->Mba_all->insert_pre_record_appl_pro_ms($insert_previous_status);
        //update the admin decision to app_pro_ms table 
      $this->Mba_all->update_appl_prom_ms($appl_pro_ms_mba,$wheresingle);
    }
    else 
    {
      // echo "reach two";
      ...
      $prev_mba=$this->Mba_all->get_app_doc_verfied_status($wheremba);

      $value_mba_status=array(
        'registration_no'=>$registration_no,
        'application_no'=>$prev_mba[0]['application_no'],
        'program_id'=>$prev_mba[0]['program_id'],
        'status'=>$prev_mba[0]['status'],
        'previous_modified_by'=>$prev_mba[0]['modified_by'],
        'previous_date_decision'=>$prev_mba[0]['modified_date'],
      );
      
      $prev_ba=$this->Mba_all->get_app_doc_verfied_status($whereba);
      ...
      //insert into log table
      $this->Mba_all->insert_pre_record_appl_pro_ms( $value_mba_status);
      $this->Mba_all->insert_pre_record_appl_pro_ms( $value_ba_status);

      //update the admin decision to app_pro_ms table 
      $this->Mba_all->update_appl_prom_ms($appl_pro_ms_mba,$wheremba);
      $this->Mba_all->update_appl_prom_ms($appl_pro_ms_ba,$whereba);
    ...
    // print_r($wheremba);
    // print_r($appl_pro_ms_mba);
    // print_r($prev_mba[0]['status']);
    // exit;
  ...
  { 
    $data['appl_data']=$this->Mba_all->fetch_csv_data();
    $this->load->view('admission/admin/layout/header');
    $this->load->view('admission/admin/excel_file_upload_view',$data);
    //$this->load->view('admission/admin/admin_tab_of_uploded_csv');
    $this->load->view('admission/admin/layout/footer');
  ...
  {  
    ...
    if (isset($_FILES)) 
    { 
      $val=$this->Mba_all->check_round_upload();
      $round='';
      if(empty($val[0]->mx))
      {
        $round=1;
      ...
      while ($csv_line = fgetcsv($fp, 1024)) 
      {
        ...
        // print_r($tot_program);
        // exit;
        $check_paid=$this->Mba_all->check_application_paid($insert_csv['registration_no']);
        ...
        { 
          ...
          $appl_no=$this->Mba_all->get_appl_no_by_pro_id($arrval);
          $appl_no2=$this->Mba_all->get_appl_no_by_pro_id($arrval2);
          ...
          if(!empty($appl_no) && !empty($appl_no2))
          {
            ...
            $check_verfied=$this->Mba_all->check_application_prom_verified($data_p);
            $check_exsit=$this->Mba_all->check_already_added_in_temp($data_p);
            $check_verfied_s=$this->Mba_all->check_application_prom_verified($data_s);
            $check_exsit_s=$this->Mba_all->check_already_added_in_temp($data_s);
            if($check_paid=='paid' && $check_verfied=='verified' && $check_exsit=='not_exist')
            {  
                $check_insert=$this->Mba_all->insert_csv_data($data_p);
                $y=$this->Mba_all->update_appl_prom_ms_call_int($call_in,$where);
            ...
            {  
                $check_insert_s=$this->Mba_all->insert_csv_data($data_s);
                $y_s=$this->Mba_all->update_appl_prom_ms_call_int($call_in,$where_s);
        ...
        { 
         ...
         if(!empty($appl_no))
         {
          ...
          'application_no'=> $appl_no,
           ...
           $check_verfied_t=$this->Mba_all->check_application_prom_verified($data);
           
           $check_exsit_t=$this->Mba_all->check_already_added_in_temp($data);
           //  echo $this->db->last_query();
           //  exit;
           if($check_paid=='paid' && $check_verfied_t=='verified' && $check_exsit_t=='not_exist')
           { 

             $check_insert=$this->Mba_all->insert_csv_data($data);
             $y=$this->Mba_all->update_appl_prom_ms_call_int($call_in,$where);
      ...
      redirect('admission/admin/dashboard/upload_excel_data');
    } 
    else 
    {
    ...
    { 
      ...
      while ($csv_line = fgetcsv($fp, 1024)) 
      {
        ...
        $data = array(
          ...
          $check_verfied=$this->Mba_all->check_application_prom_verified($data);
          ...
          {  
            $check_exsit=$this->Mba_all->check_already_added_in_temp($data);
  ...
  public function sink_data_for_interview()
  {
    ...
    $data['appl_data']=$this->Mba_all->view_call_for_interview();
    $data['call_for_interview']=$this->Mba_all->view_call_for_interview();
    $data['Synchronize_app']=$this->Mba_all->view_call_for_interview();;
    $data['Synchronize_app_view']=$this->Mba_all->synchronized_aplicant_data();
    $this->load->view('admission/admin/layout/header');
    $this->load->view('admission/admin/admin_tab_of_uploded_csv',$data);
    //$this->load->view('admission/admin/admin_tab_of_uploded_csv');
    $this->load->view('admission/admin/layout/footer');
  ...
  public function single_approve()
  {
    ...
    for($i=0; $i<count($appln); $i++)
    {
      ...
      $check=$this->Mba_all->check_application_prom_verified_application_no($application);
      if($check=='yes')
      { 
        $time_date=date("M,d,Y h:i:s A");
        $check_status_int=$this->Mba_all->check_call_for_int_status($application);
        $call_in=array(
          'call_int_status'=>'Y',
          'call_int_status_date'=>$time_date,
          'call_int_status_by'=>$verfied_done_by
