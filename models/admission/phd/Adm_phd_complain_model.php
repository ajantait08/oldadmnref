...
class Adm_phd_complain_model extends CI_Model {
  ...
  private $tabulation='mislive'; //  mis
    ...
    {
        $query = $this->db->insert('adm_phd_payment_complaints',$data);

        $query2 = $this->db2->insert('adm_phd_payment_complaints',$data);
    ...
    {
        $query = $this->db->get_where('adm_phd_payment_complaints',$where);
    ...
    {   
        ...
        $query = $this->db->get('adm_phd_payment_complaints');
        if ($query->num_rows() > 0) {
            return true;
        } else {
            return false;
    ...
    {   
        ...
        $query=$this->db->get_where('online_payment_adm_phd_log',$con);
    ...
    //     // $query = $this->db3->get('adm_phd_payment_complaints');
