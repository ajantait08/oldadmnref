...
//                                                             //
///////////////////////////////////////////////////////////////// 
class Add_phd_registration_model extends CI_Model {

    function __construct(){
      parent::__construct();
    }

    public $table_name="adm_phd_registration";
    public $adm_rapl_pro="adm_phd_reg_appl_program";
    public $dept="adm_phd_dept";
    public $program="adm_phd_program_ms";
    public $appl_ms="adm_phd_appl_ms";
    public $email_log="adm_phd_email_log";
    ...
    public function check_email_mobile_exist($email,$mob)
    {
      ...
      $query=$this->db->get_where('adm_phd_registration',$cond);
      if($query->num_rows() >0)
      {
        return 'ok';
      }
    ...
    public function total_forget_count($email)
    {
      ...
      $query=$this->db->get_where('adm_phd_email_log',$con);
      if($query->num_rows() >0)
      {
        return $query->result();
      }
    ...
    public function check_gate_paper_code_exit($cond)
    {
      ...
      $query=$this->db->get_where('adm_phd_reg_appl_program',$cond);
      if($query->num_rows() >0)
      {
        return 'ok';
      }
    ...
    {
      if($this->db->insert('adm_phd_appl_ms',$date))
      {
         return 'ok';
      }
      else
    ...
    {
      $maxid = 1;
      $row = $this->db->query('SELECT MAX(id) AS `maxid` FROM `adm_phd_registration`')->row();
      if ($row) {
  ...
  //  }
    ...
    {
      // mdn($data);
      $this->db->insert('adm_phd_registration', $data);
    ...
    {
      $myquery = "select g.verification from adm_phd_registration g where g.email='".$email."'";
    ...
    {
      $myquery = "select g.* from adm_phd_registration g where g.email= ?";
    ...
    {
      $myquery = "select g.* from adm_phd_registration g where g.mobile= ?";
    ...
    {
    $myquery = "SELECT COUNT(*) AS total FROM adm_phd_email_log e WHERE e.email_type='Link verification' and e.created_date >=curdate();";
    ...
    {
      $myquery = "select g.registration_no,g.verification from adm_phd_registration g where g.registration_no='".$val."'";
    ...
    {
      $myquery = "select g.registration_no from adm_phd_registration g where g.email='".$email."'";
      $query = $this->db->query($myquery)->row();
      if ($query) 
      {
    ...
    {
      $myquery = "select g.password from adm_phd_registration g where g.email='".$email."'";
      $query = $this->db->query($myquery)->row();
      if ($query) 
      {
    ...
    {  
      ...
      $stmt="select t.* from adm_phd_registration t where t.registration_no='".$data['registration_no']."' and t.password='".$data['password']."' and t.verification='".$data['verification']."'";
    ...
    {
      $myquery = "select g.* from adm_phd_registration g where g.registration_no='".$data['registration_no']."'";
      $query = $this->db->query($myquery);
      if ($query) 
      {
        return $query->result();
    ...
    { 
      $myquery = "SELECT t.program_id,pm.program_desc FROM adm_phd_gate_prog_eleg t
      inner JOIN adm_phd_program_ms pm ON pm.program_id=t.program_id
    ...
    { 
      $myquery = "SELECT t.* FROM adm_phd_program_ms t ";
      $query = $this->db->query($myquery);
      // echo $this->db->last_query();
      // exit;
      if ($query) 
    ...
    { 
      $myquery ="SELECT p.dept_id FROM adm_phd_program_ms p WHERE p.program_id='".$pro_id."'";
        $query = $this->db->query($myquery)->row();
        if ($query) 
        {
    ...
    { 
      $myquery = "SELECT t.* FROM adm_phd_program_ms t WHERE t.color_blind IS NULL; ";
      $query = $this->db->query($myquery);
      // echo $this->db->last_query();
      // exit;
      if ($query) 
    ...
    { 
      $myquery = "SELECT t.* FROM adm_phd_program_ms t WHERE t.pwd  IS NULL and t.color_blind IS NULL;";
      $query = $this->db->query($myquery);
      // echo $this->db->last_query();
      // exit;
      if ($query) 
    ...
    { 
      $myquery = "SELECT t.* FROM adm_phd_program_ms t WHERE t.pwd  IS NULL;";
      $query = $this->db->query($myquery);
      // echo $this->db->last_query();
      // exit;
      if ($query) 
    ...
    { 
      $myquery ="SELECT p.appl_type FROM adm_phd_registration p WHERE p.email='".$email."'";
        $query = $this->db->query($myquery)->row();
        if ($query) 
        {
    ...
    { 
        ...
        //   return false;
        // }
        $sql = "select p.cfti_flag FROM adm_phd_registration p WHERE p.registration_no=?";
    ...
    { 
      $myquery = " SELECT dm.degree_code,dm.degree_desc,pde.program_id,pde.* FROM adm_phd_program_deg_eleg pde
      INNER JOIN adm_phd_degree_ms AS dm ON dm.degree_code=pde.degree_code
    ...
    { 
      $myquery = "SELECT * FROM adm_phd_program_ms p JOIN adm_phd_dept_ms AS d ON p.dept_id=d.dept_id  WHERE p.dept_id='".$programe_code."'";
      $query = $this->db->query($myquery);
      // echo $this->db->last_query();
      // exit;
      if ($query) 
    ...
    { 
      $myquery="SELECT pde.sub_math_12th  FROM adm_phd_program_ms pde where pde.program_id= ?";
    ...
    { 
      ...
      $myquery="SELECT pde.non_min_work_exp  FROM adm_phd_program_deg_eleg pde where pde.program_id= '$data[program_id]' and pde.degree_code= '$data[degree_code]'";
      $query=$this->db->query($myquery);
    ...
    public function check_application_programme_details_row_id($reg_no,$prm_id)
     {
        $myquery = "select g.id from adm_phd_reg_appl_program g where g.registration_no='".$reg_no."' and  g.program_id='".$prm_id."'";
        $query = $this->db->query($myquery,$reg_no,$prm_id)->row();
        //  echo $this->db->last_query();
        //  exit;
        if($query) 
     ...
     public function delete_specialization($data)
     {
       ...
       $val=$this->db->delete('adm_phd_specialization',$data);
       if($val)
       {
         return true;
       }
    ...
    {
      $myquery = "SELECT p.gate_paper_desc FROM adm_phd_gate_paper p WHERE p.gate_paper_code='".$p_code."'";
      $query = $this->db->query($myquery)->row();
      if ($query) 
      {
    ...
    {
      $myquery = "SELECT * FROM adm_phd_program_ms p WHERE p.program_id='".$pro_id."'";
      $query = $this->db->query($myquery);
      if ($query) 
      {
        return $query->result();
    ...
    {
      $myquery = "SELECT p.program_desc FROM adm_phd_program_ms p WHERE p.program_id='".$pro_id."'";
      $query = $this->db->query($myquery)->row();
      if ($query) 
      {
    ...
    {
      $myquery = "SELECT d.degree_desc FROM adm_phd_degree_ms d WHERE d.degree_code='".$degree_code."'";
      // echo $this->db->last_query();
      // exit;
      $query = $this->db->query($myquery)->row();
      if ($query) 
  ...
  /////////////////////////////////////////////////////////////////                                                     
    ...
    {
      $query=$this->db->get_where('adm_phd_specialization',array('registration_no'=>$reg,'program_id'=>$pro_id));
      if($query->num_rows() >0)
      return $query->result();
    }
