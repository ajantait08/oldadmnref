...
class Adm_phd_personal_details_model extends CI_Model {
   ...
   { 
    ...
    $myquery = "SELECT t.*,p.registration_no,t.fellowship_type FROM adm_phd_fellowship_ms t 
    JOIN adm_phd_reg_appl_program AS p ON t.adm_category=p.phd_in
   ...
   { 
     $myquery = "SELECT t.*,t.fellowship_type,COUNT(t.fellowship_type) FROM adm_phd_fellowship_ms t WHERE t.assistance_type='".$ass_type."' AND t.fellowship_type !='IITGR' GROUP BY t.fellowship_type";
     $query = $this->db->query($myquery);
     if($query->num_rows() >0)
     return $query->result();
   }
   ...
   { 
    $myquery = "SELECT * FROM `adm_phd_work_exp` WHERE `registration_no` = '".$reg_no."' order by id LIMIT 1,50";
    $query = $this->db->query($myquery);
     if($query->num_rows() >0)
     return $query->result_array();
   }
   ...
   {
     $query=$this->db->query("select q.* from adm_phd_iit_ms q");
     if($query->num_rows() >0)
     return $query->result();
   }
   ...
   public function get_non_min_work_exp($reg_no)
   {
    ...
    $myquery = "select g.non_min_work_exp from adm_phd_reg_appl_program g where g.registration_no='".$reg_no."' and  g.non_min_work_exp='Y'";
   ...
   {
     $query = $this->db->query("select q.* from adm_phd_state_ms q");
     // echo $this->db->last_query();
     // exit;
     if($query->num_rows() > 0)
     return $query->result();
   ...
   public function get_tab_prog_apply($reg_no)
    ...
    public function check_tab($reg_id)
    {
      $stmt="SELECT GREATEST(IFNULL(tab1,0),IFNULL(tab2,0),IFNULL(tab3,0),IFNULL(tab4,0)) highest
        FROM adm_phd_tab WHERE registration_no='$reg_id'";
    ...
    {
      $myquery = "select g.reg_id from adm_phd_registration g where g.email='".$email."'";
      $query = $this->db->query($myquery)->row();
      if ($query) 
      {
        return $query->reg_id;
    ...
    {
      $myquery = "select g.registration_no,g.reg_id,g.first_name,g.middle_name,g.last_name,g.category,g.pwd,g.mobile,g.email,g.dob,g.btech_flag,g.math_stat_flag,g.email  from adm_phd_registration g where g.email='".$email."'";
      $query = $this->db->query($myquery);
      if ($query) 
      {
        return $query->result();
    ...
    {
      $query=$this->db->get_where('adm_phd_specialization_ms',array('program_id'=>$pro_id));
      if($query->num_rows() >0)
      return $query->result();
    }
