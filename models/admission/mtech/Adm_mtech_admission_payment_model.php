...
/////////////////////////////////////////////////////////////////
//                                                             //
//               mtech query model is start                      //
//                                                             //
//                                                             //
/////////////////////////////////////////////////////////////////
class Adm_mtech_admission_payment_model extends CI_Model {
...
//private $table_pay='paybeta';
  ...
  //  $this->db3 = $CI->load->database($this->tablepay_beta, TRUE); // mtechportal
  ...
  public $cat="adm_mtech_fellowship";
  public $qualification="adm_mtech_qulaification";
  public $work_exp="adm_mtech_work_exp";
  public $document="adm_mtech_doc";
  public $adm_mtech_appl_selected = "adm_mtech_appl_selected";

   function get_fee_details($reg_id,$email,$contact_no)
   {
      $this->db->select('fee_to_be_paid');
      $query = $this->db->get_where('online_payment_adm_mtech_final_fee',['reg_id' => $reg_id, 'email' => $email, 'contact_no' => $contact_no]); // stud_final_with_fee
      if($query->num_rows() > 0)
      {
   ...
   function insert_mis_pay_adm_pay_detail($payment_details,$cond)
   {
    ...
    $querystmt = $this->db->get_where('online_payment_admission_mtech_final_fee',$cond);
    if($querystmt->num_rows() > 0)
    {
      $msginsertmtech="mtechexsit";
    }
    else
    {
      $query = $this->db->insert('online_payment_admission_mtech_final_fee',$payment_details); // mtechportal db

      if($this->db->affected_rows() > 0)
      {
        $msginsertmtech="mtechok";
      }
      else
      {
        $msginsertmtech="mtechnot";
      }
    }

    $querystmt2 = $this->db2->get_where('online_payment_admission_mtech_final_fee',$cond);
    ...
    {
      $query2 = $this->db2->insert('online_payment_admission_mtech_final_fee',$payment_details); // mis db
    ...
    $querystmt3 = $this->db4->get_where('online_payment_admission_mtech_final_fee',$cond);
    ...
    {
      $query3 = $this->db4->insert('online_payment_admission_mtech_final_fee',$payment_details); // pay db
     ...
     return [$msginsertmtech,$msginsertpay,$msginsertmis];
   ...
   public function get_registration($data)
   {
    ...
    public function get_adm_mtech_appl_ms_selected($reg_no,$program_selected)
    {
      $sql = "select a.*,b.* from adm_mtech_appl_ms a inner join adm_mtech_appl_selected b on a.registration_no = b.registration_no where a.registration_no = '".$reg_no."' and b.program_id = '".$program_selected."'";
    ...
    public function get_adm_mtech_reg_appl_program_selected($reg_no,$program_selected)
    {
      $query=$this->db->get_where($this->adm_mtech_appl_selected,array('registration_no'=>$reg_no,'program_id'=>$program_selected));
      if($query->num_rows() >0)
      return $query->result_array();
    }
   ...
   // now check in mtech branch selected then
   function check_mtech_mtechba_branch_newadmission($reg_id,$admn_type)
   {
   ...
   function get_mtech_select_branch_by_stu_newadmission($where_data)
   {
       $query3 = $this->db3->get_where("mtech_select_branch_by_stu",$where_data);
