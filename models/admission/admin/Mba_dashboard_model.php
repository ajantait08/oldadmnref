...
class Mba_dashboard_model extends CI_Model
{
    ...
    public function get_appl_no_by_pro_id($val)
    {
     ...
     //   print_r($val['prog']);
     //   exit;
      $myquery = "SELECT t.application_no FROM adm_mba_reg_appl_program t WHERE t.registration_no='".$val['reg']."' AND t.program_id='".$val['prog']."'";
    ...
    public function update_appl_prom_ms_call_int_syn($appl)
    {
      ...
      $stmt="UPDATE `adm_mba_reg_appl_program` SET `admin_decision_status` = 'CI', `admin_desicion_sta_desc` = 'Call for interview' WHERE `application_no` = '".$appl."'";
    ...
    public function view_call_for_interview_wrong_query()
    {
        ...
        m.email,m.mobile,m.dob,m.category,m.gender,f.cat_score,f.cat_percentile,l.call_int_status,l.admin_decision_status FROM adm_mba_reg_appl_program l
        left JOIN adm_mba_appl_ms AS m ON m.registration_no=l.registration_no
        left JOIN adm_mba_fellowship AS f ON f.registration_no=l.registration_no 
    ...
    {
        $stmt="SELECT y.Registration_no,g.application_no,g.program_id,y.Applicant_name,y.Category,y.PWD,y.Gender,
        y.CAT_Reg_no,y.CAT_Percentile,g.admin_decision_status,g.admin_desicion_sta_desc,g.call_int_status FROM adm_mba_temp_shortlist y 
        JOIN adm_mba_reg_appl_program as g ON y.Registration_no=g.registration_no ";
        $query = $this->db->query($stmt);
        if ($query->num_rows() > 0)
         return $query->result();
        else
    ...
    public function synchronized_aplicant_data()
    {
        $stmt="SELECT y.Registration_no,g.application_no,g.program_id,y.Applicant_name,y.Category,y.PWD,y.Gender,
        y.CAT_Reg_no,y.CAT_Percentile,g.admin_decision_status,g.admin_desicion_sta_desc,g.call_int_status FROM adm_mba_temp_shortlist y 
        JOIN adm_mba_reg_appl_program as g ON y.Registration_no=g.registration_no  WHERE g.admin_decision_status='CI'";
        $query = $this->db->query($stmt);
        if ($query->num_rows() > 0)
         return $query->result();
        else
    ...
    {
        $stmt="SELECT * FROM adm_mba_reg_appl_program m WHERE m.application_no='".$data."' AND m.call_int_status='Y'";
        $query = $this->db->query($stmt);
    ...
    {
        $stmt="SELECT * FROM adm_mba_csv_data_upload_temp c WHERE c.registration_no='".$data['registration_no']."' AND c.application_no='".$data['application_no']."' AND c.`status`='A'";
        $query = $this->db->query($stmt);
        if ($query->num_rows() > 0)
          return "exist";
        else
    ...
    {
        $stmt="SELECT * FROM adm_mba_appl_ms m WHERE m.payment_status='Y' AND m.registration_no='".$data."'";
        $query = $this->db->query($stmt);
        if ($query->num_rows() > 0)
          return "paid";
        else
    ...
    {
        $stmt="SELECT * FROM adm_mba_reg_appl_program r WHERE r.application_no='".$data['application_no']."' AND r.program_id='".$data['program_id']."' AND r.`status`='v'";
        $query = $this->db->query($stmt);
        if ($query->num_rows() > 0)
          return "verified";
        else
    ...
    {   
        ...
        // $stmt="SELECT * FROM adm_mba_reg_appl_program r WHERE r.application_no='".$app."' AND r.`status`='V'";
        // $query = $this->db->query($stmt);
        // echo $this->db->last_query();
        // exit;
    ...
    public function fetch_csv_data()
    {
        ...
        FROM adm_mba_csv_data_upload_temp p 
        left JOIN adm_mba_appl_ms AS m ON m.registration_no=p.registration_no
        left JOIN adm_mba_fellowship AS f ON f.registration_no=p.registration_no 
    ...
    {
        $query = $this->db->query("select MAX(upload_round) as mx from adm_mba_csv_data_upload_temp");
        if ($query->num_rows() > 0)
          return $query->result();
        else
         return false;
    ...
    {
        $this->db->insert('adm_mba_csv_data_upload_temp',$insert_data);
