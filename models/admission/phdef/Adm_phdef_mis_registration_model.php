...
class Adm_phdef_mis_registration_model extends CI_Model
{
    ...
    function validate_login($reg_id,$prog_selected,$appl_type)
    {
        ...
        FROM adm_phdef_appl_selected a
        INNER JOIN adm_phdef_appl_ms b ON a.registration_no=b.registration_no
        INNER JOIN adm_phdef_address c ON a.registration_no=c.registration_no AND c.address_type='P'
        left join adm_phdef_fellowship d ON a.registration_no=d.registration_no
        INNER JOIN adm_phdef_program_ms e ON a.program_id=e.program_id
        INNER JOIN adm_phdef_reg_appl_program f ON a.program_id=f.program_id AND a.registration_no=f.registration_no
        WHERE a.registration_no = '$reg_id' and d.fellowship_serial_no = 'one' AND b.appl_type='Full time' AND a.program_id='$prog_selected' GROUP BY a.registration_no";
  ...
  function insert_upload_error_logs($upload_array){
    $this->db->insert('upload_error_logs',$upload_array);
  ...
  function insert_initial_tab_data($tab_data){
    $this->db->insert('adm_phdef_mis_registration_tab',$tab_data);
    #echo $this->db->last_query(); die();
    if ($this->db->insert_id()) {
       return true;
    }
  ...
  function get_prev_other_hostel_details($reg_id){
     $query = $this->db->select('*')
                      ->from('adm_phdef_mis_registration_hostel_details')
                      ->where('registration_no',$reg_id)
                      ->get();
  ...
  function get_prev_stu_account_details($reg_id){
    $query = $this->db->select('*')
                     ->from('adm_phdef_mis_reg_student_account_details')
                     ->where('registration_no',$reg_id)
                     ->get();
...
function check_tab5_status($reg_id){
  $query = $this->db->select('*')
  ->get_where('adm_phdef_mis_registration_tab',array('registration_no'=>$reg_id,'tab5'=>'5'));
  if($query->num_rows() > 0)
  {
    return true;
  }
  ...
  function get_mis_reg_education_details($reg_id){
    $sql = "select * from adm_phdef_mis_reg_education_details where registration_no = ? order by id desc LIMIT 1";
  ...
  function get_prev_personal_details($reg_id){
    $sql = "select * from adm_phdef_mis_reg_personal_details_temp where registration_no = ?";
    $query = $this->db->query($sql,array($reg_id));
    if ($query->num_rows() > 0) {
         return $query->result_array();
    }
  ...
  function delete_prev_personal_details($reg_id){
      ...
      if ($reg_id != '') {
        ...
        $this->db->delete('adm_phdef_mis_reg_personal_details_temp');
  ...
  function delete_prev_fee_details($reg_id){
    ...
    if ($reg_id != '') {
      $this->db->where('registration_no',$reg_id);
      $this->db->delete('adm_phd_mis_reg_fee_details');
      if ($this->db->affected_rows() > 0) {
        return true;
      }
      else{
  ...
  function delete_prev_education_details($reg_id){
    ...
    if ($reg_id != '') {
      $this->db->where('registration_no',$reg_id);
      $this->db->delete('adm_phdef_mis_reg_education_details');
      if ($this->db->affected_rows() > 0) {
        return true;
      }
      else{
...
function delete_prev_hostel_details($reg_id){
  ...
  if ($reg_id != '') {
    $this->db->where('registration_no',$reg_id);
    $this->db->delete('adm_phdef_mis_registration_hostel_details');
    if ($this->db->affected_rows() > 0) {
      return true;
    }
    else{
...
function delete_prev_account_details($reg_id){
  ...
  if ($reg_id != '') {
    $this->db->where('registration_no',$reg_id);
    $this->db->delete('adm_phdef_mis_reg_student_account_details');
    if ($this->db->affected_rows() > 0) {
      return true;
    }
    else{
  ...
  function get_marksheet_by_sno($sno,$reg_id){
      if ($sno != '') {
        $query = $this->db->select('marks_sheet')
                          ->from('adm_phdef_mis_reg_stu_prev_certificate')
                          ->where('sno',$sno)
                          ->where('registration_no',$reg_id)
                          ->get();
        if ($query->num_rows() > 0) {
...
function get_dynamic_sub_by_sno($sno,$reg_id){
  ...
  function check_education_details_already_exists($reg_id){
    $sql = "select * from adm_phdef_mis_reg_education_details where registration_no = ?";
    $query = $this->db->query($sql,array($reg_id));
    if ($query->num_rows() > 0) {
      return $query->result_array();
    }
  ...
  function insert_personal_details_log($data_log){
    $this->db->insert('adm_phdef_mis_reg_personal_details_log',$data_log);
    //echo $this->db->last_query(); die();
    if ($this->db->insert_id() != '') {
  ...
  function insert_education_details_log($data_log){
    $this->db->insert('adm_phdef_mis_reg_education_details_log',$data_log);
    #echo $this->db->last_query(); die();
    if ($this->db->insert_id() != '') {
      return $this->db->insert_id();
    }
  ...
  function insert_hostel_details_log($data_log){
    $this->db->insert('adm_phdef_mis_registration_hostel_details_log',$data_log);
    //echo $this->db->last_query(); die();
    if ($this->db->insert_id() != '') {
      return $this->db->insert_id();
    }
  ...
  function get_prev_stu_fee_details($registration_no){
    $sql = "select * from adm_phdef_mis_reg_fee_details where registration_no = ?";
  ...
  function insert_account_details_log($data_log){
    $this->db->insert('adm_phdef_mis_reg_student_account_details_log',$data_log);
    //echo $this->db->last_query(); die();
    if ($this->db->insert_id() != '') {
      return $this->db->insert_id();
    }
  ...
  function insert_fee_details_log($data_log){
    $this->db->insert('adm_phdef_mis_reg_fee_details_log',$data_log);
    //echo $this->db->last_query(); die();
    if ($this->db->insert_id() != '') {
      return $this->db->insert_id();
    }
  ...
  function get_mis_reg_prev_education_details($reg_id){
    $sql = "select * from adm_phdef_mis_reg_stu_prev_education where registration_no = ?";
    $query = $this->db->query($sql,array($reg_id));
    if ($query->num_rows() > 0) {
      return $query->result_array();
    }
  ...
  function get_stu_prev_certificate_details($sno){
    if ($sno != '') {
    $sql = "select * from adm_phdef_mis_reg_stu_prev_certificate where sno = ?";
  ...
  function insert_stu_prev_certificate_details_log($form_data){
    $this->db->insert('adm_phdef_mis_reg_stu_prev_certificate_log', $form_data);
    //echo $this->db->last_query(); die();
    return $this->db->insert_id();
  }
  ...
  function update_stu_prev_certificate($form_data){
    ...
    $this->db->update('adm_phdef_mis_reg_stu_prev_certificate',$data_update);
    if ($this->db->affected_rows() > 0) {
       return true;
    }
    else{
  ...
  function update_branch_mis_reg_edu($update_array,$where_data){
    ...
    $this->db->update('adm_phdef_mis_reg_education_details',$update_array);
    #echo $this->db->last_query(); die();
    if ($this->db->affected_rows() >= 0) {
       return true;
    }
  ...
  public function get_mis_reg_prev_certificate_details($reg_id){
    $sql = "select * from adm_phdef_mis_reg_stu_prev_certificate where registration_no = ?";
    $query = $this->db->query($sql,array($reg_id));
    if ($query->num_rows() > 0) {
      return $query->result_array();
    }
  ...
  public function get_photopath_from_personal_details($reg_id){
      $query = $this->db->select('photo_path')
                        ->from('adm_phdef_mis_reg_personal_details_temp')
                        ->where('registration_no',$reg_id)
                        ->get();
      //echo $this->db->last_query(); die();
...
function get_state_list()
  ...
  function get_admn_no_from_stud_final_with_fee($reg_id)
    ...
    function get_current_user_tab_status($registration_no){
        $query = $this->db->select('*')
                          ->from('adm_phdef_mis_registration_tab')
    ...
    function insert_current_user_tab_status($phd_mis_reg_tab_status){
         $this->db->insert('adm_phdef_mis_registration_tab',$phd_mis_reg_tab_status);
        //  echo $this->db->last_query(); exit;
         return $this->db->insert_id();
         //exit;
    }
    ...
    function get_phdef_education_details($registration_id) {
        ...
        // $query = $this->db5->get_where('adm_phd_qulaification',['registration_no' => $registration_id]);
         ...
         a.created_by,a.created_date,b.doc_id,b.doc_path
          from adm_phdef_qulaification a INNER JOIN adm_phdef_doc b ON a.registration_no=b.registration_no AND
         b.doc_id=(CASE
     ...
     function get_photo_signature_phdef($registration_id) {
         $query = $this->db->query("SELECT * from adm_phdef_doc a where a.doc_id in('photo','signature') and a.registration_no=? ",[$registration_id]);
     ...
     function insert_personal_details($data_insert){
        ...
        $this->db->insert('adm_phdef_mis_reg_personal_details_temp',$data_insert);
        //echo $this->db->last_query(); die();
     ...
     function update_mis_reg_tab_status($data_phd_mis_reg,$registration_no){
      ...
      $this->db->update('adm_phdef_mis_registration_tab',$data_phd_mis_reg);
      if($this->db->affected_rows() > 0)
      {
        return true;
      }
     ...
     function save_phdef_mis_reg_parent_details($save_parent_details){
        $this->db->insert('adm_phdef_mis_reg_parent_details',$save_parent_details);
        //echo $this->db->last_query(); die();
        return $this->db->insert_id();
     }

     function save_hostel_info($data){
       $this->db->insert('adm_phdef_mis_registration_hostel_details',$data);
        //echo $this->db->last_query(); die();
        return $this->db->insert_id();
     }

     function save_account_details($stu_account_details){
      $this->db->insert('adm_phdef_mis_reg_student_account_details',$stu_account_details);
      return $this->db->insert_id();
     }

     function save_phdef_mis_reg_prev_parent_details_log($save_parent_details){
      $this->db->insert('adm_phdef_mis_reg_edit_parent_details_log',$save_parent_details);
      return $this->db->insert_id();
   }

     function get_mis_reg_parent_details($reg_no){
        $query = $this->db->select('*')
                        ->from('adm_phdef_mis_reg_parent_details')
                        ->where('registration_no',$reg_no)
                        ->get();
         if($query->num_rows() > 0){
            return $query->result_array();
     ...
     function get_education_details($registration_no){
      $query = $this->db->select('*')
                      ->from('adm_phdef_mis_reg_education_details')
                      ->where('registration_no',$registration_no)
                      //->where('admn_no',$admn_no)
                      ->get();
      if($query->num_rows() > 0){
     ...
     function get_prev_education_details($registration_no){
      $query = $this->db->select('*')
                      ->from('adm_phdef_mis_reg_stu_prev_education')
                      ->where('registration_no',$registration_no)
                      //->where('admn_no',$admn_no)
                      ->get();
      if($query->num_rows() > 0){
     ...
     function get_fee_details($registration_no){
      $query = $this->db->select('*')
                      ->from('adm_phdef_mis_reg_fee_details')
                      ->where('registration_no',$registration_no)
                      //->where('admn_no',$admn_no)
                      ->get();
      if($query->num_rows() > 0){
     ...
     function save_stu_fee_details($stu_fee_details){
      $this->db->insert('adm_phdef_mis_reg_fee_details',$stu_fee_details);
      //echo $this->db->last_query(); die();
      return $this->db->insert_id();
     }
     ...
     function insertbatch_stu_prev_education_mis_registration($data)//9
    {
        ...
        // echo $this->db->last_query(); die();
        if($this->db->insert_batch('adm_phdef_mis_reg_stu_prev_education',$data)){
          return true;
        }
        else{
          return false;
    ...
    function insertbatch_stu_prev_certificate_mis_registration($data)//10
    {
        ...
        //  echo $this->db->last_query(); die();
        if($this->db->insert_batch('adm_phdef_mis_reg_stu_prev_certificate',$data))
        {
          return true;
        }
        else{
    ...
    function insertbatch_stu_prev_education_log($data){
      $this->db->insert_batch('adm_phdef_mis_reg_stu_prev_education_log',$data);
      //echo $this->db->last_query(); die();
      if($this->db->insert_batch('adm_phdef_mis_reg_stu_prev_education_log',$data))
        {
          return true;
        }
        else{
    ...
    function insertbatch_stu_prev_certificate_log($data){
      ...
      if($this->db->insert_batch('adm_phdef_mis_reg_stu_prev_certificate_edit_log',$data))
      {
        return true;
      }
      else{
    ...
    function remove_prev_edu_mis_reg_main_table($registration_no){
      if ($registration_no != '') {
        $this->db->where('registration_no', $registration_no);
        if($this->db->delete('adm_phdef_mis_reg_stu_prev_education'))
        {
          return true;
        }
        else{
    ...
    function remove_prev_cert_mis_reg_main_table($registration_no){
      if ($registration_no != '') {
        $this->db->where('registration_no', $registration_no);
        if($this->db->delete('adm_phdef_mis_reg_stu_prev_certificate'))
        {
          return true;
        }
        else{
    ...
    function remove_prev_parent_mis_reg_main_table($registration_no){
      if ($registration_no != '') {
        ...
        //echo $this->db->last_query(); die();
        if($this->db->delete('adm_phdef_mis_reg_parent_details'))
        {
          return true;
        }
        else{
    ...
    function insert_mis_reg_edu_details($data){
      ...
      // echo $this->db->last_query(); die();
    if ($this->db->insert('adm_phdef_mis_reg_education_details',$data)) {
      return true;
    }
    else{
      return false;
