...
//                                                             //
/////////////////////////////////////////////////////////////////
class Adm_mba_admission_payment_model extends CI_Model {

  private $tabulation='mislive'; //this for misdev when do live the use below one
...
//private $table_pay='paybeta';
  ...
  //  $this->db3 = $CI->load->database($this->tablepay_beta, TRUE); // mbaportal
  ...
  public $adm_mba_appl_selected = "adm_mba_appl_selected";

   function get_fee_details($reg_id,$email,$contact_no)
   {
      $this->db->select('fee_to_be_paid');
      $query = $this->db->get_where('online_payment_adm_mba_final_fee',['reg_id' => $reg_id, 'email' => $email, 'contact_no' => $contact_no]); // stud_final_with_fee
      if($query->num_rows() > 0)
      {
   ...
   function insert_mis_pay_adm_pay_detail($payment_details,$cond)
   {
    ...
    $querystmt = $this->db->get_where('online_payment_admission_mba_final_fee',$cond);
    if($querystmt->num_rows() > 0)
    {
      $msginsertmba="mbaexsit";
    }
    else
    {
      $query = $this->db->insert('online_payment_admission_mba_final_fee',$payment_details); // mbaportal db
    ...
    $querystmt2 = $this->db2->get_where('online_payment_admission_mba_final_fee',$cond);
    ...
    {
      $query2 = $this->db2->insert('online_payment_admission_mba_final_fee',$payment_details); // mis db
    ...
    $querystmt3 = $this->db4->get_where('online_payment_admission_mba_final_fee',$cond);
    ...
    {
      $query3 = $this->db4->insert('online_payment_admission_mba_final_fee',$payment_details); // pay db
     ...
     return [$msginsertmba,$msginsertpay,$msginsertmis];
   ...
   {
        $this->db->where($where_data);
        $this->db->update('adm_mba_tab',$update_array);

       return TRUE;

   }

   function update_payment_appl_ms_details($data,$reg,$prog)
   {
     ...
     $this->db->update('adm_mba_appl_selected', $data , $where);
    ...
    public function get_adm_mba_appl_ms_selected($reg_no,$program_selected)
    {
      $sql = "select a.*,b.*,a.dob as dob from adm_mba_appl_ms a inner join adm_mba_appl_selected b on a.registration_no = b.registration_no where a.registration_no = '".$reg_no."' and b.program_id = '".$program_selected."'";
    ...
    public function get_adm_mba_reg_appl_program_selected($reg_no,$program_selected)
    {
      $query=$this->db->get_where($this->adm_mba_appl_selected,array('registration_no'=>$reg_no,'program_id'=>$program_selected));
      if($query->num_rows() >0)
      return $query->result_array();
    }
   ...
   // now check in mba branch selected then
   function check_mba_mbaba_branch_newadmission($reg_id,$admn_type)
   {
   ...
   function get_mba_select_branch_by_stu_newadmission($where_data)
   {
       $query3 = $this->db3->get_where("mba_select_branch_by_stu",$where_data);
   ...
   // now check in mba branch selected then
   // changes for mtech 2021
 function get_branch_from_mtech($branch_id)
 {
   $this->db3->select('branch_name');
   $this->db3->where('branch_id',$branch_id); // from newadmission db
   $query=$this->db3->get('dept_course_branch_mtech_new');
   $row=$query->row_array();
   return $row;
 }
 function get_dept_from_mtech($dept_id)
 {
   $this->db3->select('dept_name');
   $query = $this->db3->get_where('dept_course_branch_mtech_new',['dept_id' => $dept_id]); // from newadmission db
   return $query->row_array();
 }
// end changes for mtech 2021
