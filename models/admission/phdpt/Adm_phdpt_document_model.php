...
class Adm_phdpt_document_model extends CI_Model {
    ...
    public $gate="adm_phdpt_fellowship";
    ...
    public function check_color_blind_doc($reg_no)
    {
      ...
      $myquery = "SELECT * FROM adm_phdpt_reg_appl_program p
      JOIN adm_phdpt_program_ms d ON d.dept_id=p.dept_id AND p.phd_in=d.adm_category
    ...
    public function insert_gate_details($data)
    {
      ...
      $this->db2->insert($this->gate,$data);
      //  echo $this->db->last_query();
      //   exit;
      if($query)
      {
    ...
    public function remove_document_val($condition)
    {
      ...
      $this->db2->delete($this->document,$condition);
      //  echo $this->db->last_query();
      //   exit;
      if($query)
      {
    ...
    public function get_appl_no_by_reg_id($reg)
    {
      ...
      $myquery = "select g.appl_no from adm_phdpt_appl_ms g where g.reg_no='".$reg."'";   // appl_no field is not there--- changed here by ----MADAN---on---23-02-2022
