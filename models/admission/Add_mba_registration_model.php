...
//                                                             //
///////////////////////////////////////////////////////////////// 
class Add_mba_registration_model extends CI_Model {
  private $tabulation='mislive';  //this for mislive when do live the use below one
    ...
    public $table_name="adm_mba_registration";
    public $adm_rapl_pro="adm_mba_reg_appl_program";
    public $dept="adm_mba_dept";
    public $program="adm_mba_program_ms";
    public $appl_ms="adm_mba_appl_ms";
    public $email_log="adm_mba_email_log";
    

    public function check_email_mobile_exist($cond)
    {
     
      $query=$this->db->get_where('adm_mba_registration',$cond);
      if($query->num_rows() >0)
      {
        return 'ok';
      }
    ...
    {
    $myquery = "SELECT COUNT(*) AS total FROM adm_mba_email_log e WHERE e.email_type='Link verification' and e.created_date >=curdate();";
    ...
    $query=$this->db->get_where('adm_mba_email_log',$con);
    if($query->num_rows() >0)
    {
     return $query->result();
    }
    ...
    { 
      //$query=$this->db->get_where($this->dept,array('registration_no'=>$reg_no));
      $query=$this->db->get_where($this->dept,array('status'=>'A'));
      if($query->num_rows() >0)
      return $query->result_array();
    }
    public function get_program($data)
    { 
      if($data['betch']=='Y' and $data['math_stat']=='Y')
      {
        $query=$this->db->get_where($this->program,array('status'=>'A'));
      }
      else if($data['betch']=='Y' and $data['math_stat']=='N')
      {
        $query=$this->db->get_where($this->program,array('status'=>'A'));
       
      }
      else if($data['betch']=='N' and $data['math_stat']=='Y')
      {
        $query=$this->db->get_where($this->program,array('program_id'=>'mba','status'=>'A'));
      }
      else {
        $query=$this->db->get_where($this->program,array('program_id'=>'non','status'=>'A'));
    ...
    {
      $myquery = "select g.* from adm_mba_registration g where g.email= ?";
    ...
    {
      $myquery = "select g.* from adm_mba_registration g where g.mobile= ?";
    ...
    {
      // $myquery = "select g.* from adm_mba_registration g where g.email= ? and g.registration_no= ?";
      // $query = $this->db->query($myquery,$email,$reg);
      // if($query) 
      // {
      //   return "ok";
      // }
    ...
    {
      $myquery = "select g.id from adm_mba_reg_appl_program g where g.registration_no='".$reg_no."' and  g.program_id='".$prm_id."'";
    ...
    {
      $myquery = "select g.password from adm_mba_registration g where g.email='".$email."'";
      $query = $this->db->query($myquery)->row();
      if ($query) 
      {
    ...
    {
      $myquery = "select g.registration_no from adm_mba_registration g where g.email='".$email."'";
      $query = $this->db->query($myquery)->row();
      if ($query) 
      {
    ...
    {
      $maxid = 1;
      $row = $this->db->query('SELECT MAX(id) AS `maxid` FROM `adm_mba_registration`')->row();
      if ($row) {
